...
import { getTemplates } from '@/lib/platform/services';

export const metadata: Metadata = {
  title: 'القوالب',
  description: 'اختر قالباً جاهزاً لبدء موقعك',
  alternates: { canonical: '/builder/templates' },
};
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-brand-black py-8 md:py-12">
      <div className="max-w-[1400px] mx-auto px-4 md:px-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-bold">اختر قالباً جاهزاً لبدء موقعك</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {categories.map((cat) => (
            <Link
              key={cat.id}
              href={`/builder/templates${cat.id !== 'all' ? `?category=${cat.id}` : ''}`}
              className={`px-4 py-2 rounded-full text-sm font-black transition-all ${
                activeCategory === cat.id
                  ? 'bg-brand-gradient text-white'
                  : 'bg-white dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
            >
              {cat.label}
            </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {templates.map((tpl, i) => (
            ...
            >
              {/* Thumbnail */}
              <div className="relative aspect-video bg-gradient-to-br from-brand-cyan/10 to-brand-purple/10 overflow-hidden">
                {tpl.thumbnail ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <span className="text-4xl font-black text-gradient">{tpl.name.charAt(0)}</span>
                  </div>
                )}
                {tpl.isPremium && (
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <Link href={`/builder/edit/${tpl.id}?template=true`} className="w-11 h-11 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center hover:bg-white/30 transition-all" title="معاينة">
                    <Eye className="w-5 h-5 text-white" />
                  </Link>
                  <Link href={`/builder/create?template=${tpl.slug}`} className="w-11 h-11 rounded-xl bg-brand-gradient flex items-center justify-center hover:shadow-glow-cyan transition-all" title="استخدام">
