...
import { ScrollText, RefreshCw, Download, Printer, Truck, TrendingDown, CheckCircle2 } from 'lucide-react';
...
export default function StatementView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSuppliers(supData.map((s: any) => ({ id: String(s.id), name: s.name || s.nameAr || '---' })));
      setOrders(poData);
  ...
  const entries = useMemo<Entry[]>(() => {
    ...
    type Raw = { id: string; ref: string; kind: 'invoice' | 'payment'; date: string; debit: number; credit: number };
    const raw: Raw[] = [];
    mine.forEach((o: any) => {
      ...
      if (paid > 0) {
        raw.push({ id: `pay-${o.id}`, ref: `دفعة — ${ref}`, kind: 'payment', date, debit: 0, credit: paid });
      }
    });
  ...
  const exportCSV = useCallback(() => {
    ...
    const body = entries.map(e => [e.date, e.ref, e.kind === 'invoice' ? 'فاتورة' : 'دفعة', e.debit, e.credit, e.balance]);
    const csvContent = [headers, ...body].map(row => row.join(',')).join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `statement-${selectedName}.csv`;
    link.click();
      ...
      {selectedName && (
        <div className="grid grid-cols-3 gap-3 mt-3">
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            ...
            <div className="text-lg sm:text-xl font-black text-slate-900">ج.م {fmt(totals.debit)}</div>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <div className="text-lg sm:text-xl font-black text-slate-900">ج.م {fmt(totals.credit)}</div>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            {entries.map((e) => (
              <InvRow key={e.id}>
                ...
                <div className="col-span-3 min-w-0">
                  <div className="font-bold text-slate-900 text-xs sm:text-sm truncate">{e.ref}</div>
                </div>
