...
import {
  RefreshCw, PackageX, AlertTriangle, Download,
} from 'lucide-react';
              ...
              {paginated.map((p) => (
                <InvRow key={p.id} muted={p.stock === 0}>
                  <div className="col-span-4 min-w-0">
                    ...
                    <div className="text-xs font-medium text-slate-500 mt-0.5 truncate">{p.sku}</div>
                  </div>
                  <div className="col-span-2 pr-4 text-slate-600 text-xs sm:text-sm truncate">{p.category}</div>
