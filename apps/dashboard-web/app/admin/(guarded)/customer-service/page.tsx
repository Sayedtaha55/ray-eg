...
import {
  Headphones, User, Clock, Trash2, Loader2, Filter, Send,
  AlertTriangle, CheckCircle2, MessageCircle, HelpCircle, ChevronDown, ChevronUp,
} from 'lucide-react';
            ...
            return (
              ...
              >
                <div className="p-6 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : item.id)}>
                  ...
                  <p className="text-slate-400 font-bold text-sm leading-relaxed text-right line-clamp-2">{item.message}</p>
                  ...
                  <div className="border-t border-white/5 p-6 space-y-4">
                    ...
                    {item.adminReply && (
                      <div className="bg-green-500/5 p-4 rounded-2xl text-right border border-green-500/10">
                        ...
                        {item.repliedAt && <p className="text-[10px] text-slate-500 font-bold mt-2">{formatDate(item.repliedAt)}</p>}
                      </div>
                    )}
                      ...
                      {['OPEN', 'IN_PROGRESS', 'RESOLVED', 'CLOSED'].map((st) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all disabled:opacity-50 ${
                            item.status === st ? 'bg-white/10 text-white' : 'bg-white/5 text-slate-500 hover:text-white'
                          }`}
                        >
