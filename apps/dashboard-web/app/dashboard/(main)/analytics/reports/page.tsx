...
function periodRange(p: PeriodKey): { from: string; to: string; label: string } {
  ...
  switch (p) {
    ...
    default:
      return { from: '', to: d(now), label: 'الكل' };
  }
}
  ...
  return (
    <div className="px-4 py-2.5">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span
          className="font-mono tabular-nums font-black text-slate-800 text-xs sm:text-sm whitespace-nowrap"
          dir="ltr"
        >
          {fmt2(value)}
          {suffix}
...
function ReportsContent() {
  ...
  const kpis = useMemo(() => {
    const revenue =
      Number(data.revenue?.total_revenue ?? data.revenue?.net_profit ?? 0) ||
      (data.revenue?.revenue || []).reduce((s: number, l: any) => s + Number(l.amount || 0), 0);
    const prevRevenue =
      Number(data.prevRevenue?.total_revenue ?? 0) ||
      (data.prevRevenue?.revenue || []).reduce((s: number, l: any) => s + Number(l.amount || 0), 0);
    const expenses = (data.expenses || [])
      .filter((e: any) => inPeriod(e.date, revRange))
  ...
  const sections = useMemo(() => {
    ...
    const inRangeOrders = orders.filter((o: any) =>
      inPeriod(o.created_at || o.createdAt || o.date, revRange)
    );
    const orderTotal = inRangeOrders.reduce(
      (s, o) => s + Number(o.total || o.total_amount || o.grand_total || 0),
      0
    );
    ...
    inRangeOrders.forEach((o) => {
      ...
      byPayment.set(
        pm,
        (byPayment.get(pm) || 0) + Number(o.total || o.total_amount || o.grand_total || 0)
      );
    });
    ...
    const transactions: any[] = (data.transactions || []).filter((t: any) =>
      inPeriod(t.date, revRange)
    );
  ...
  const sectionHeader = (id: string) => {
    switch (id) {
      case 'sales':
        return [`ج.م ${fmt(sections.orderTotal)}`, `${sections.orders.length} عملية`];
      case 'purchases':
        return [`ج.م ${fmt(sections.purchasesTotal)}`, `مسدد ج.م ${fmt(sections.purchasesPaid)}`];
      case 'expenses':
        return [`ج.م ${fmt(kpis.expenses)}`, `${sections.expenses.length} بند`];
      case 'profits':
        return [
          `${kpis.netProfit >= 0 ? 'ربح' : 'خسارة'} ج.م ${fmt(Math.abs(kpis.netProfit))}`,
          'للفترة',
        ];
      case 'customers':
        return [`مستحق ج.م ${fmt(sections.totalCustomerDue)}`, 'من كشف الأعمار'];
      case 'suppliers':
        return [`مستحق ج.م ${fmt(sections.totalVendorDue)}`, `${sections.suppliers.length} مورد`];
      case 'cash':
        return [
          `سيولة ج.م ${fmt(sections.wallets.reduce((s: number, w: any) => s + Number(w.balance || 0), 0))}`,
          `${sections.wallets.length} حساب`,
        ];
      case 'inventory':
        return [
          `ج.م ${fmt(sections.stockValue)}`,
          `${sections.lowStock} ناقص / ${sections.outStock} نافد`,
        ];
      case 'branches':
        return ['حسب البيانات', ''];
      case 'employees':
        return ['حسب الصلاحيات', ''];
      case 'taxes':
        return [
          `صافي ج.م ${fmt(sections.netTax)}`,
          `${sections.submittedReturns.length}/${sections.taxReturns.length} مقدمة`,
        ];
      default:
        return ['', ''];
    }
    ...
    switch (id) {
      case 'sales': {
        ...
        return (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
            <div>
              <h4 className="font-black text-slate-700 text-xs mb-2 px-4">
              ...
              {pm.size === 0 ? (
                ...
                [...pm.entries()].map(([k, v]) => (
                  <MiniBar key={k} label={k} value={v} max={maxOf(pm)} color="bg-emerald-500" />
                ))
              )}
            </div>
            <div className="grid grid-cols-3 gap-3 h-fit">
              ...
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                ...
                <div className="font-black text-slate-900 text-sm mt-1">
                  ج.م{' '}
                  {fmt(sections.orders.length ? sections.orderTotal / sections.orders.length : 0)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4">
            ...
            {sections.bySupplier.size === 0 ? (
              <p className="text-slate-400 text-xs font-bold">
                لا مشتريات في الفترة — فواتير الشراء تظهر من أوامر الشراء عند الاستلام
              </p>
            ) : (
              [...sections.bySupplier.entries()]
                .sort((a, b) => b[1] - a[1])
                .slice(0, 5)
                .map(([k, v]) => (
                  <MiniBar
      ...
      case 'expenses': {
        ...
        const prevExpenses = (data.expenses || [])
          .filter((e: any) => !inPeriod(e.date, revRange) && inPeriod(e.date, prevRange(period)))
          .reduce((s: number, e: any) => s + Number(e.amount || 0), 0);
        return (
          <div className="p-4">
            ...
            {prevExpenses > 0 && (
              <p className="text-[11px] font-bold text-slate-400 mt-3 px-1">
                الفترة السابقة المقارنة: ج.م {fmt2(prevExpenses)} —{' '}
                {kpis.expenses >= prevExpenses ? 'ارتفاع' : 'انخفاض'}{' '}
                {prevExpenses
                  ? Math.abs(((kpis.expenses - prevExpenses) / prevExpenses) * 100).toFixed(1)
                  : 0}
                %
              </p>
            )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            {[
              ...
              { label: 'المصروفات', v: kpis.expenses, c: 'text-rose-700' },
              {
                label: 'صافي الربح',
                v: kpis.netProfit,
                c: kpis.netProfit >= 0 ? 'text-slate-900' : 'text-rose-700',
              },
            ].map((x) => (
          ...
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                ...
                <div className="font-black text-rose-700 text-sm mt-1">
                  ج.م {fmt(sections.totalCustomerDue)}
                </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                <div className="text-[11px] font-bold text-slate-400">عملاء عليهم مستحق</div>
                <div className="font-black text-slate-900 text-sm mt-1">
                  {(data.aging || []).filter((a: any) => sections.agingRowTotal(a) > 0).length}
                </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                ...
                <a
                  ...
                  className="font-black text-sky-700 text-xs mt-1 inline-block hover:underline"
                >
                  العملاء والمدينون →
                </a>
              </div>
            ...
            {(data.aging || []).length > 0 && (
              <>
                ...
                {[...(data.aging || [])]
                  .sort((a: any, b: any) => sections.agingRowTotal(b) - sections.agingRowTotal(a))
                  .slice(0, 5)
                  .map((a: any, i: number) => (
                    <MiniBar
                      key={i}
                      label={a.entity_name || a.entityName || '—'}
                      value={sections.agingRowTotal(a)}
                      max={Math.max(
                        1,
                        ...(data.aging || []).map((x: any) => sections.agingRowTotal(x))
                      )}
                      color="bg-rose-500"
                    />
                  ))}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                ...
                <div className="font-black text-slate-900 text-sm mt-1">
                  ج.م{' '}
                  {fmt(
                    sections.wallets.reduce((s: number, w: any) => s + Number(w.balance || 0), 0)
                  )}
                </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
        ...
        return (
          <div className="p-4 space-y-3">
            ...
            <a
              ...
              className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50"
            >
              <span>تقارير المرتجعات</span>
              <span className="text-slate-400">عرض التفاصيل ←</span>
            </a>
          </div>
        );
      case 'branches':
        return (
          <div className="p-4">
            <p className="text-slate-400 text-xs font-bold">
              تقارير الفروع تظهر عند تفعيل أكثر من فرع — النظام يعمل حاليًا بفرع واحد موحد.
            </p>
          </div>
        );
      case 'employees':
        return (
          <div className="p-4">
            <p className="text-slate-400 text-xs font-bold">
              تقارير الموظفين (المبيعات والتحصيل والأداء) تظهر حسب الصلاحيات عند تفعيل وحدة الموارد
              البشرية.
            </p>
          </div>
        );
      case 'taxes': {
        const activeRates = sections.taxRates.filter(
          ...
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                ...
                <div className="font-black text-slate-900 text-sm mt-1">
                  {sections.submittedReturns.length} / {sections.taxReturns.length}
                </div>
              </div>
              <div className="bg-slate-50 rounded-xl p-3 text-center">
                <div className="text-[11px] font-bold text-slate-400">صافي الضريبة المستحقة</div>
                <div
                  className={`font-black text-sm mt-1 ${sections.netTax >= 0 ? 'text-rose-700' : 'text-emerald-700'}`}
                >
                  ج.م {fmt2(sections.netTax)}
                </div>
              </div>
            </div>
            <a
        ...
        <div className="px-4 sm:px-6 py-5 max-w-[1400px] mx-auto flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              مركز واحد لكل الأرقام — من الفواتير والمدفوعات والمصروفات والمخزون، حسب الفترة
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 mt-4 pb-10 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-2 flex-wrap">
          ...
          {['الفرع', 'الموظف', 'طريقة الدفع المتقدم'].map((f) => (
            <span
              key={f}
              title="يتاح عند توفر البيانات"
              className="px-3 py-1.5 rounded-full bg-slate-50 text-slate-300 text-[11px] font-bold whitespace-nowrap border border-dashed border-slate-200"
            >
              {f}
            </span>
          ))}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <p className="text-xs font-bold text-slate-400 mb-1 flex items-center gap-1.5">
              <TrendingUp size={13} className="text-emerald-500" /> إجمالي الإيرادات
            </p>
            <p className="text-lg font-black text-emerald-700">ج.م {fmt(kpis.revenue)}</p>
            {kpis.prevRevenue > 0 && (
              <p
                className={`text-[11px] font-bold mt-0.5 ${kpis.revGrowth >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
              >
                {kpis.revGrowth >= 0 ? '↑' : '↓'} {Math.abs(kpis.revGrowth).toFixed(1)}% عن الفترة
                السابقة
              </p>
            )}
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <p className="text-xs font-bold text-slate-400 mb-1 flex items-center gap-1.5">
              <TrendingDown size={13} className="text-rose-500" /> إجمالي المصروفات
            </p>
            <p className="text-lg font-black text-rose-700">ج.م {fmt(kpis.expenses)}</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <p className="text-xs font-bold text-slate-400 mb-1 flex items-center gap-1.5">
              <Wallet size={13} /> صافي الربح
            </p>
            <p
              className={`text-lg font-black ${kpis.netProfit >= 0 ? 'text-slate-900' : 'text-rose-700'}`}
            >
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <p className="text-xs font-bold text-slate-400 mb-1 flex items-center gap-1.5">
              <Droplets size={13} className="text-cyan-500" /> التدفق النقدي
            </p>
            <p
              className={`text-lg font-black ${kpis.cashNet >= 0 ? 'text-cyan-700' : 'text-rose-700'}`}
            >
              ج.م {fmt(kpis.cashNet)}
            </p>
            {(kpis.inflow > 0 || kpis.outflow > 0) && (
              <p className="text-[11px] font-bold text-slate-400 mt-0.5">
                داخل ج.م {fmt(kpis.inflow)} • خارج ج.م {fmt(kpis.outflow)}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-sky-50 border border-sky-100 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <span className="text-[11px] font-black text-sky-800">
            كل الأرقام هنا من نفس مصدر المالية والمحاسبة — أي فاتورة أو دفعة أو مصروف مرحَّل يظهر
            هنا تلقائيًا. القوائم التفصيلية في{' '}
            <a href="/dashboard/finance/financial-reports" className="underline">
              القوائم المالية
            </a>
            .
          </span>
