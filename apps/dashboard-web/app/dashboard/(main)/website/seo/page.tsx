...
import { Search, TrendingUp, CheckCircle2, AlertCircle, Loader2, RefreshCw, Filter, ChevronDown, Info, Target, BookOpen, Zap, Link2, ChevronRight, Lightbulb, XCircle, Calendar, Globe, BarChart3, ExternalLink, Copy, Sparkles, ArrowUp, ArrowDown } from 'lucide-react';
...
const TYPE_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
  page: { label: 'صفحة', color: 'text-blue-600', bg: 'bg-blue-100', icon: <Globe size={12} /> },
  product: { label: 'منتج', color: 'text-purple-600', bg: 'bg-purple-100', icon: <Sparkles size={12} /> },
  category: { label: 'تصنيف', color: 'text-amber-600', bg: 'bg-amber-100', icon: <BarChart3 size={12} /> },
  blog: { label: 'مقال', color: 'text-green-600', bg: 'bg-green-100', icon: <Sparkles size={12} /> },
};
...
export default function SeoPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      // const data = await apiRequest(`/seo/shop/${sid}`);
  ...
  const seoGuide: WebsiteGuideData = {
    purpose: 'تحسين محركات البحث (SEO) لموقعك مع تحليل وتوصيات ذكية لتحسين الترتيب.',
    whenToUse: 'استخدم هذه الصفحة لتحليل SEO لموقعك وتطبيق التوصيات لتحسين الترتيب.',
    whatsInside: [
      'تحليل SEO للصفحات والمنتجات',
      'نقاط قوة وضعف SEO',
      'توصيات ذكية للتحسين',
    ...
    steps: [
      ...
      { title: 'راقب النتائج', description: 'راقب تحسن الترتيب مع الوقت' }
    ],
    bestPractices: [
    ...
    relatedLinks: [
      ...
      { label: 'المدونة', onClick: () => window.location.href = '/dashboard/website/blog' }
    ]
  };
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="text-right">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900">تحسين محركات البحث</h1>
            <button onClick={() => setGuideOpen(true)} className="p-1.5 rounded-full text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all" title="معلومات / Info">
              <Info size={18} />
            </button>
          </div>
          <p className="text-sm font-bold text-slate-400 mt-1">تحسين SEO للموقع</p>
        </div>
      </div>
              ...
              <div key={item.id} className="p-4 hover:bg-slate-50 transition-all">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5" dir="ltr">{item.url}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all" title="زيارة">
      ...
      {guideOpen && (
        <InfoDrawer title="تحسين محركات البحث" onClose={() => setGuideOpen(false)}>
          <WebsiteGuideContent guide={seoGuide} />
        </InfoDrawer>
      )}
    </div>
  );
