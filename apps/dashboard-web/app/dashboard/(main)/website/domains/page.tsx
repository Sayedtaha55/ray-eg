      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 font-bold mt-2">اربط نطاق مخصص بموقعك</p>
          </div>
          <button className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-slate-900 text-white font-black text-sm hover:bg-slate-800 transition-all">
        ...
        {domains.length > 0 ? (
          <div className="space-y-4">
            {domains.map((domain) => {
              ...
              return (
                <div key={domain.id} className="p-6 rounded-3xl bg-white border border-slate-100">
                  ...
                  {domain.status === 'pending' && domain.dnsRecords && (
                    <div className="mt-4 p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10">
                      <p className="text-xs font-bold text-amber-600 mb-3">سجلات DNS المطلوبة:</p>
                      <div className="space-y-2">
                        {domain.dnsRecords.map((record: any, i: number) => (
                          <div key={i} className="flex items-center gap-3 text-xs font-bold">
                            <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-600 font-black">{record.type}</span>
                            <span className="text-slate-500" dir="ltr">{record.host}</span>
                            <span className="text-slate-400">→</span>
                            <span className="text-slate-600" dir="ltr">{record.value}</span>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <button className="text-xs font-black text-blue-600 hover:underline">تحقق</button>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-12 text-center">
            ...
            <p className="text-slate-500 font-bold text-sm mb-6">اربط نطاقك المخصص أو استخدم نطاق فرعي مجاني</p>
            <button className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-slate-900 text-white font-black text-sm hover:bg-slate-800 transition-all">
              <Plus className="w-5 h-5" />
              إضافة نطاق
            </button>
          </div>
