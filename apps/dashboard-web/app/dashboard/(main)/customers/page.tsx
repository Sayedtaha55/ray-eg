...
export default function CustomersPage() {
  ...
  const sendPromotion = useCallback(async (customerId: string) => {
    try {
      ...
      alert('فشل إرسال العرض الترويجي');
    }
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'متوسط الطلب',
        value: `ج.م ${avgOrderValue.toFixed(0)}`,
        icon: Star,
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white p-4 rounded-2xl border border-slate-100">
            ...
            <p className="text-[10px] text-slate-400 font-bold mt-1">
              +{analytics.newCustomersThisMonth || 0} جديد هذا الشهر
            </p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-slate-100">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-[10px] text-slate-400 font-bold mt-1">
              {analytics.returningCustomers || 0} عميل عائد
            </p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-slate-100">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xl font-black text-slate-900">
              {analytics.avgVisitsPerCustomer || 0}
            </p>
      ...
      {analytics && (
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 ${activeSection === 'all' ? 'bg-slate-900 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
          >
            <Users size={14} /> كل العملاء
          </button>
          <button
            onClick={() => setActiveSection('top')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 ${activeSection === 'top' ? 'bg-amber-500 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
          >
            <Crown size={14} /> أفضل العملاء
          </button>
          <button
            onClick={() => setActiveSection('atRisk')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 ${activeSection === 'atRisk' ? 'bg-amber-500 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 ${activeSection === 'churned' ? 'bg-red-500 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}
          >
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={bulkDelete}
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-700 font-bold text-xs hover:bg-red-100 transition-all"
            >
              <Trash2 size={14} />
              حذف
            </button>
              ...
              {paginatedCustomers.map((customer) => {
                ...
                return (
                  <tr key={customer.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                    ...
                    <td className="p-4">
                      <div className="font-bold text-slate-900 text-sm">
                        ج.م {customer.averageOrderValue.toFixed(0)}
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg transition-all ${customer.status === 'active' ? 'bg-red-50 text-red-600 hover:bg-red-500 hover:text-white' : 'bg-green-50 text-green-600 hover:bg-green-500 hover:text-white'}`}
                          title={customer.status === 'active' ? 'حظر' : 'تفعيل'}
                        >
          ...
          <div className="relative ml-auto w-full max-w-lg h-full bg-white shadow-2xl overflow-y-auto">
            ...
            <div className="sticky top-0 bg-white border-b border-slate-200 p-5 flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                <div
                  className={`w-11 h-11 rounded-xl flex items-center justify-center font-black text-lg ${customerDetail?.stats?.isChurned ? 'bg-red-100 text-red-500' : customerDetail?.stats?.isAtRisk ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}
                >
                  {selectedDetailCustomer.name?.charAt(0) || 'U'}
                </div>
                <div>
                  <h3 className="font-black text-base text-slate-900">
                    {customerDetail?.customer?.name || selectedDetailCustomer.name}
                  </h3>
                  <p className="text-[10px] text-slate-500 font-bold">
                    {customerDetail?.stats?.isChurned
                      ? 'توقف عن الشراء'
                      : customerDetail?.stats?.isAtRisk
                        ? 'معرض للتوقف'
                        : 'عميل نشط'}
                  </p>
                </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="bg-purple-50 rounded-xl p-3 text-center">
                    ...
                    <p className="text-lg font-black text-slate-900">
                      {customerDetail.stats?.avgOrderValue || 0}
                    </p>
                  ...
                  <div className="bg-slate-50 rounded-xl p-3">
                    ...
                    <div className="flex items-end gap-1.5 h-12">
                      {customerDetail.monthlySpending.map((d: any, i: number) => {
                        const max = Math.max(
                          ...customerDetail.monthlySpending.map((x: any) => x.total),
                          1
                        );
                        return (
                    ...
                    <div className="space-y-1.5">
                      {customerDetail.favoriteProducts.map((p: any, i: number) => (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-black text-slate-400 bg-white px-2 py-0.5 rounded-lg">
                            {p.count}x
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="space-y-1.5 max-h-48 overflow-y-auto">
                    {customerDetail.orders && customerDetail.orders.length > 0 ? (
                      customerDetail.orders.map((o: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center justify-between bg-white border border-slate-200 rounded-xl p-2.5"
                        >
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-7 h-7 rounded-lg flex items-center justify-center ${o.status === 'DELIVERED' ? 'bg-green-100 text-green-600' : o.status === 'CONFIRMED' ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}
                            >
                              <ShoppingBag className="w-3.5 h-3.5" />
                            </div>
                            <div>
                              <p className="text-xs font-bold text-slate-700">
                                ج.م {o.total?.toLocaleString()}
                              </p>
                          ...
                          <span
                            className={`text-[10px] font-bold px-2 py-1 rounded-lg ${o.status === 'DELIVERED' ? 'bg-green-50 text-green-600' : o.status === 'CONFIRMED' ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-500'}`}
                          >
                            {o.status}
                          </span>
                        </div>
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    onClick={() => sendPromotion(selectedDetailCustomer.id)}
                    className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-black text-white bg-purple-500 hover:bg-purple-600 transition-all"
                  >
                    <Megaphone className="w-4 h-4" /> إرسال عرض ترويجي
                  </button>
                  <button
                    onClick={() => {
                      toggleStatus(selectedDetailCustomer.id, selectedDetailCustomer.status);
                      closeCustomerDetail();
                    }}
                    className={`px-3 py-2.5 rounded-xl text-xs font-black transition-all ${selectedDetailCustomer.status === 'active' ? 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-green-50 text-green-500 hover:bg-green-500 hover:text-white'}`}
                  >
