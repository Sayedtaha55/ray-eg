...
const WebsiteReturnsPage: React.FC<Props> = ({ shopId, isArabic }) => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const websiteOrders = (Array.isArray(data) ? data : []).filter((o: any) => o?.source !== 'pos' && o?.source !== 'POS');
      setOrders(websiteOrders);
    } catch (e: any) {
      setError(String(e?.message || 'Error loading orders'));
      setOrders([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setError(String(e?.message || 'Error loading returns'));
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="text-slate-900 font-black text-xl">{isArabic ? 'مرتجعات الموقع' : 'Website Returns'}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
          ...
          <div className="mt-2 text-slate-900 font-black text-lg">{isArabic ? 'ج.م' : 'EGP'} {Number(summary.totalReturnedAmount || 0).toLocaleString()}</div>
        </div>
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
          ...
          <div className="mt-2 text-slate-900 font-black text-lg">
            {summary.hasReturns ? (isArabic ? 'يوجد مرتجعات' : 'Has returns') : (isArabic ? 'لا يوجد' : 'None')}
          </div>
        </div>
      </div>

      {rows.length === 0 && !loading ? (
        <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 text-slate-600 font-black text-sm">
          {isArabic ? 'لا توجد مرتجعات' : 'No returns found'}
        </div>
      ) : null}

      {rows.length > 0 && (
        <div className="overflow-x-auto touch-auto no-scrollbar">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.returnId} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-5"><span className="font-black text-slate-900 text-sm">{isArabic ? 'ج.م' : 'EGP'} {Number(r.totalAmount || 0).toLocaleString()}</span></td>
                  <td className="p-5 text-left">
                    <button type="button" onClick={() => { setSelectedRow(r); setDetailsOpen(true); }}
      ...
      <Modal isOpen={detailsOpen} onClose={() => { setDetailsOpen(false); setSelectedRow(null); }} title={isArabic ? 'تفاصيل المرتجع' : 'Return Details'} size="lg">
        <div className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              <div className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">{isArabic ? 'رقم الطلب' : 'Order #'}</div>
              <div className="mt-2 text-white font-black text-base sm:text-lg">#{selectedRow?.orderShortId || '-'}</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              <div className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">{isArabic ? 'الإجمالي' : 'Total'}</div>
              <div className="mt-2 text-white font-black text-base sm:text-lg">{isArabic ? 'ج.م' : 'EGP'} {Number(selectedRow?.totalAmount || 0).toLocaleString()}</div>
            </div>
          </div>
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
            <div className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">{isArabic ? 'العناصر المرتجعة' : 'Returned Items'}</div>
            <div className="mt-3 space-y-2">
              {(Array.isArray(selectedRow?.items) ? selectedRow?.items : []).map((it: any, idx: number) => (
                <div key={idx} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 bg-black/20 border border-white/10 rounded-xl p-3 sm:p-4">
                  <div className="text-white font-bold text-base sm:text-sm truncate">{it?.product?.name || (isArabic ? 'منتج' : 'Product')}</div>
                  <div className="text-slate-200 font-black text-sm sm:text-xs shrink-0">
                    {Number(it?.quantity || 0)} x {isArabic ? 'ج.م' : 'EGP'} {Number(it?.unitPrice || 0)}
                  </div>
                </div>
              ))}
              {!Array.isArray(selectedRow?.items) || selectedRow?.items?.length === 0 ? (
                <div className="text-slate-300 font-bold text-base sm:text-sm">{isArabic ? 'لا توجد عناصر' : 'No items'}</div>
              ) : null}
            </div>
          </div>
        </div>
