...
const SharedProductsTab: React.FC<Props> = ({ 
  ...
  const pageTitle = activityVocab.productsTabLabel;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onAdd}
          className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg font-semibold hover:bg-black transition-all"
        >
          <Plus size={20} />
          {activityVocab.addProductButton}
        </button>
      </div>

      {products.length === 0 ? (
        <div className="text-center py-20 text-slate-400">
          <p className="font-semibold">{activityVocab.emptyProductsMessage}</p>
          <button
            onClick={onAdd}
            className="mt-4 px-6 py-3 bg-cyan-500 text-black rounded-lg font-semibold hover:brightness-110 transition-all"
          >
            {activityVocab.addProductButton}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.slice(0, renderCount).map((product) => (
            ...
            >
              ...
              <p className="text-slate-500 font-semibold mb-4">{t('business.sharedProducts.currency')} {product.price}</p>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2 bg-slate-100 rounded-lg font-semibold hover:bg-slate-200 transition-all"
                >
                  <Edit size={16} />
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2 bg-purple-100 text-purple-600 rounded-lg font-semibold hover:bg-purple-200 transition-all"
                >
                  <Tag size={16} />
