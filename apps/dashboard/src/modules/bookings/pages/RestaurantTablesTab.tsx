...
const RestaurantTablesTab: React.FC<Props> = ({ shop, onSaved }) => {
    ...
    return (
      <div className="bg-white p-12 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center justify-center py-20">
          <span className="text-slate-400 font-semibold">جاري تحميل الطاولات...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 text-right" dir="rtl">
      <div className="bg-white p-6 sm:p-8 md:p-10 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          ...
          <button
            type="button"
            onClick={openAdd}
            className="shrink-0 flex items-center gap-2 px-5 py-3 rounded-lg font-semibold text-xs text-white hover:opacity-90 transition-all"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-center">
            ...
            <div className="text-xs font-semibold text-slate-500 mt-1">إجمالي الطاولات</div>
          </div>
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-center">
            <div className="text-2xl font-bold text-emerald-700">{counts.available}</div>
            <div className="text-xs font-semibold text-emerald-500 mt-1">متاحة</div>
          </div>
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-center">
            <div className="text-2xl font-bold text-amber-700">{counts.reserved}</div>
            <div className="text-xs font-semibold text-amber-500 mt-1">محجوزة</div>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-100 p-4 text-center">
            <div className="text-2xl font-bold text-slate-600">{counts.maintenance}</div>
            <div className="text-xs font-semibold text-slate-500 mt-1">غير متاحة</div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((table) => {
              ...
              return (
                <div
                  key={table.id}
                  className="rounded-lg border border-slate-200 p-5 hover:shadow-md transition-all bg-white"
                >
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div>
                      <div className="font-bold text-slate-900 text-sm">{table.name}</div>
                      {table.section && (
                        <div className="text-xs font-semibold text-slate-500 mt-1">{table.section}</div>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    disabled={saving}
                    className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg border text-xs font-semibold transition-all disabled:opacity-60 ${meta.bg} ${meta.color}`}
                  >
                    {meta.icon}
                    {meta.label}
                  </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md">
          <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500">اسم الطاولة</label>
                <input
                  value={form.name}
                  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  placeholder="مثال: طاولة 1 / تراس 3"
                  className="mt-2 w-full px-4 py-3 rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 focus:outline-none focus:border-slate-400"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-500">عدد الكراسي</label>
                  <input
                    type="number"
                    min={1}
                    value={form.capacity}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500">القسم / القاعة</label>
                  <input
                    value={form.section}
            ...
            <div className="p-6 pt-0">
              <button
                ...
                onClick={submitForm}
                disabled={saving}
                className="w-full py-3.5 rounded-lg bg-slate-900 text-white font-semibold hover:bg-black transition-all disabled:opacity-60"
              >
