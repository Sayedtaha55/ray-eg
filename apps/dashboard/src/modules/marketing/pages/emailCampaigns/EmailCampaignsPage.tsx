...
import { Mail, Plus, Search, Eye, MousePointerClick, X, Send } from 'lucide-react';
import { useTranslation } from 'react-i18next';

type Props = { shopId: string; shop?: any };

type EmailCampaign = { id: string; subject: string; recipients: number; opened: number; clicked: number; status: 'draft' | 'sent' | 'scheduled'; date: string };

const EmailCampaignsPage: React.FC<Props> = ({ shopId, shop }) => {
  const { t, i18n } = useTranslation();
  const isArabic = String(i18n.language || '').toLowerCase().startsWith('ar');
  const [search, setSearch] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [campaigns, setCampaigns] = useState<EmailCampaign[]>([
    { id: '1', subject: isArabic ? 'عرض خاص للعملاء' : 'Special offer for customers', recipients: 500, opened: 320, clicked: 150, status: 'sent', date: '2026-07-28' },
    { id: '2', subject: isArabic ? 'منتجات جديدة' : 'New products arrived', recipients: 800, opened: 0, clicked: 0, status: 'scheduled', date: '2026-08-01' },
  ]);
  ...
  return (
    <div className="bg-white p-4 sm:p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div><h3 className="text-xl sm:text-2xl md:text-3xl font-black">{isArabic ? 'حملات البريد' : 'Email Campaigns'}</h3><p className="mt-1 text-xs sm:text-sm font-bold text-slate-400">{isArabic ? 'حملات التسويق عبر البريد الإلكتروني' : 'Email marketing campaigns'}</p></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: isArabic ? 'متوسط الفتح' : 'Avg Open Rate', value: campaigns.length ? Math.round(campaigns.reduce((s, c) => s + (c.recipients ? (c.opened / c.recipients) * 100 : 0), 0) / campaigns.length) + '%' : '0%', color: 'bg-amber-50 text-amber-600' },
        ].map((s, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-2xl border border-slate-100">
            <div className={`p-2 rounded-xl ${s.color}`}><Mail size={20} /></div>
        ...
        {filtered.map((c) => (
          <div key={c.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 hover:border-slate-200 transition-colors">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-blue-50 text-blue-600"><Mail size={20} /></div>
              <div><p className="font-bold text-sm">{c.subject}</p><p className="text-xs text-slate-400">{new Date(c.date).toLocaleDateString(isArabic ? 'ar-EG' : 'en-US')}</p></div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-center"><p className="text-xs text-slate-400">{isArabic ? 'المستلمين' : 'Recipients'}</p><p className="font-bold text-sm">{c.recipients}</p></div>
              <div className="text-center"><p className="text-xs text-slate-400"><Eye size={12} className="inline" /></p><p className="font-bold text-sm">{c.opened}</p></div>
              <div className="text-center"><p className="text-xs text-slate-400"><MousePointerClick size={12} className="inline" /></p><p className="font-bold text-sm">{c.clicked}</p></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <input placeholder={isArabic ? 'عنوان البريد' : 'Email subject'} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm" />
              <textarea placeholder={isArabic ? 'محتوى البريد' : 'Email content'} rows={4} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm" />
