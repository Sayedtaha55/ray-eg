    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 sm:p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900">{isArabic ? 'مركز التسويق' : 'Marketing Center'}</h3>
              <p className="text-xs text-slate-500 font-semibold">{isArabic ? 'إدارة الحملات والإعلانات الممولة من مكان واحد' : 'Manage campaigns and sponsored ads all in one place'}</p>
            </div>
          </div>
          <button
            onClick={() => setShowCreateModal(true)}
            className="px-4 py-2.5 bg-slate-900 text-white rounded-lg font-semibold text-xs flex items-center justify-center gap-2 hover:bg-black transition-all"
          >
            <Plus size={16} />
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {sectionTabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-xs whitespace-nowrap transition-all ${
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {quickActions.map((action) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 p-4 rounded-lg bg-gradient-to-br ${action.color} text-white font-semibold text-xs hover:scale-105 transition-transform shadow-sm`}
                >
                  {action.icon}
                  <span className="text-center">{action.label}</span>
                  <span className="text-xs opacity-80 text-center">{action.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          {campaigns.length === 0 ? (
            <div className="py-20 text-center">
              ...
              <p className="font-bold text-xl text-slate-300 mb-2">{isArabic ? 'لا توجد حملات بعد' : 'No campaigns yet'}</p>
              ...
              <button
                onClick={() => setShowCreateModal(true)}
                className="px-6 py-3 bg-slate-900 text-white rounded-lg font-semibold text-xs inline-flex items-center gap-2 hover:bg-black transition-all"
              >
                <Plus size={16} />
            ...
            <div className="space-y-4">
              {campaigns.map((campaign) => {
                ...
                return (
                  <div key={campaign.id} className="bg-slate-50 p-5 rounded-lg border border-slate-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${platform.bgColor} ${platform.color}`}>
                          {platform.icon}
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 text-sm">{campaign.name}</p>
                          <p className="text-xs text-slate-500 font-semibold">{isArabic ? platform.label.ar : platform.label.en}</p>
                        </div>
                      </div>
                      <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${status.bgColor} ${status.color}`}>
                        {status.icon}
                    ...
                    <div className="grid grid-cols-4 gap-3 mb-3">
                      <div>
                        <p className="text-xs font-semibold text-slate-500">{isArabic ? 'الظهور' : 'Impr.'}</p>
                        <p className="text-sm font-bold text-slate-900">{campaign.impressions.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-500">{isArabic ? 'النقرات' : 'Clicks'}</p>
                        <p className="text-sm font-bold text-slate-900">{campaign.clicks.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-500">{isArabic ? 'التحويلات' : 'Conv.'}</p>
                        <p className="text-sm font-bold text-slate-900">{campaign.conversions.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-500">{isArabic ? 'الإنفاق' : 'Spent'}</p>
                        <p className="text-sm font-bold text-slate-900">{t('business.reports.currency')} {campaign.spent.toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-slate-500">{progress.toFixed(0)}%</span>
                    </div>
                  </div>
                );
              })}
      ...
      {activeSection === 'audience' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <p className="text-xs text-slate-500 font-semibold mb-6">{isArabic ? 'استهدف العملاء حسب سلوكهم ومستوى تفاعلهم' : 'Target customers by behavior and engagement'}</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {audienceSegments.map((seg, i) => (
                <div key={i} className="bg-slate-50 p-5 rounded-lg border border-slate-200 hover:shadow-md transition-all cursor-pointer">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${seg.color} text-white`}>
                    {seg.icon}
                  </div>
                  <p className="text-2xl font-bold text-slate-900">{seg.value}</p>
                  <p className="text-xs font-semibold text-slate-500 mt-1">{seg.label}</p>
                  <button className="mt-3 text-xs font-semibold text-blue-500 flex items-center gap-1 hover:gap-2 transition-all">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                { label: isArabic ? 'إعادة استهداف العملاء غير النشطين' : 'Retarget Inactive Customers', desc: isArabic ? 'تواصل مع العملاء الذين لم يزورا متجرك مؤخراً' : 'Reach customers who haven\'t visited recently' },
                ...
                <div key={i} className="flex items-center justify-between bg-slate-50 p-4 rounded-lg border border-slate-200 hover:bg-slate-100 transition-all cursor-pointer">
                  <div>
                    <p className="text-sm font-bold text-slate-900">{opt.label}</p>
                    <p className="text-xs text-slate-500 font-semibold">{opt.desc}</p>
                  </div>
      ...
      {activeSection === 'tools' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <p className="text-xs text-slate-500 font-semibold mb-6">{isArabic ? 'اربط حساباتك على منصات الإعلانات وتابع أداءها من هنا' : 'Connect your ad platform accounts and track performance here'}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <button
                  ...
                  className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200 hover:shadow-md transition-all text-right"
                >
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${platform.color}`}>
                    {platform.icon}
                  </div>
                  <div className="flex-1">
                    <p className="font-bold text-sm text-slate-900">{platform.name}</p>
                    <p className="text-xs text-slate-500 font-semibold">{platform.desc}</p>
                  </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className="flex flex-col items-center gap-2 p-5 bg-slate-50 rounded-lg border border-slate-200 hover:shadow-md transition-all text-center"
                >
                  ...
                  <span className="text-xs text-slate-500 font-semibold">{tool.desc}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-xl p-6 md:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-slate-900">{isArabic ? 'حملة جديدة' : 'New Campaign'}</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500 mb-2 block">{isArabic ? 'اسم الحملة' : 'Campaign Name'}</label>
                <input
                  type="text"
                  placeholder={isArabic ? 'مثال: حملة الصيف' : 'e.g. Summer Sale'}
                  className="w-full px-4 py-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-semibold focus:outline-none focus:border-purple-300"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-500 mb-2 block">{isArabic ? 'المنصة' : 'Platform'}</label>
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(PLATFORM_CONFIG).map(([key, config]) => (
                    <button
                      key={key}
                      className={`flex flex-col items-center gap-1 p-3 rounded-lg border-2 border-transparent hover:border-purple-200 transition-all ${config.bgColor}`}
                    >
                      <span className={config.color}>{config.icon}</span>
                      <span className="text-xs font-semibold text-slate-600">{isArabic ? config.label.ar : config.label.en}</span>
                    </button>
                  ))}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-2 block">{isArabic ? 'الميزانية' : 'Budget'}</label>
                  <input
                    type="number"
                    placeholder="0"
                    className="w-full px-4 py-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-semibold focus:outline-none focus:border-purple-300"
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-2 block">{isArabic ? 'تاريخ البداية' : 'Start Date'}</label>
                  <input
                    type="date"
                    className="w-full px-4 py-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-semibold focus:outline-none focus:border-purple-300"
                  />
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500 mb-2 block">{isArabic ? 'الجمهور المستهدف' : 'Target Audience'}</label>
                <select className="w-full px-4 py-3 bg-slate-50 rounded-lg border border-slate-200 text-sm font-semibold focus:outline-none focus:border-purple-300">
              ...
              <button
                ...
                className="w-full py-4 bg-gradient-to-l from-purple-600 to-pink-600 text-white rounded-lg font-semibold text-sm hover:opacity-90 transition-all"
              >
