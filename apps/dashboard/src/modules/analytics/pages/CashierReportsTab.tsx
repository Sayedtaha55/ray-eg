  ...
  if (totalOrders === 0) {
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 md:p-12 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex flex-wrap gap-2 mb-8">
            {rangeOptions.map((opt) => (
              <button
                key={opt.key}
                onClick={() => setRange(opt.key)}
                className={`px-4 py-2 rounded-lg text-xs font-semibold ${range === opt.key ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}
              >
                {opt.label}
              </button>
            ))}
          </div>
          <div className="py-16 md:py-24 text-center text-slate-300 font-semibold">{t('business.reports.noData')}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="bg-white p-6 md:p-10 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-slate-500 text-xs font-semibold mt-1">{t('business.reports.cashierReportsDesc')}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {rangeOptions.map((opt) => (
              <button
                key={opt.key}
                onClick={() => setRange(opt.key)}
                className={`px-3 py-2 md:px-4 rounded-lg text-xs font-semibold ${range === opt.key ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}
              >
                {opt.label}
              </button>
            ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white p-5 md:p-7 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            <ShoppingCart size={16} className="text-cyan-500" />
            <p className="text-slate-500 font-semibold text-xs">{t('business.reports.totalOrdersInPeriod')}</p>
          </div>
        ...
        <div className="bg-white p-5 md:p-7 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-slate-500 font-semibold text-xs">{t('business.reports.avgOrdersPerHour')}</p>
          </div>
          <span className="text-2xl font-bold">{avgOrdersPerHour}</span>
        </div>
        <div className="bg-white p-5 md:p-7 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp size={16} className="text-green-500" />
            <p className="text-slate-500 font-semibold text-xs">{t('business.reports.bestSellingProduct')}</p>
          </div>
          <span className="text-sm font-bold truncate block">{bestProduct?.name || '—'}</span>
          <span className="text-xs text-slate-500 font-semibold">{bestProduct?.totalQty || 0} {t('business.reports.quantity')}</span>
        </div>
        <div className="bg-white p-5 md:p-7 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-2">
            <TrendingDown size={16} className="text-orange-500" />
            <p className="text-slate-500 font-semibold text-xs">{t('business.reports.slowestSellingProduct')}</p>
          </div>
          <span className="text-sm font-bold truncate block">{slowestProduct?.name || '—'}</span>
          <span className="text-xs text-slate-500 font-semibold">{slowestProduct?.totalQty || 0} {t('business.reports.quantity')}</span>
        </div>
      </div>

      {/* Hourly distribution chart */}
      <div className="bg-white p-6 md:p-10 rounded-xl border border-slate-200 shadow-sm">
        <h4 className="text-lg md:text-xl font-bold mb-1">{t('business.reports.hourlyDistribution')}</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-white p-6 md:p-8 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="font-bold text-sm">{t('business.reports.busyHours')}</h4>
          </div>
          <div className="space-y-2">
            {busyHours.map((h, i) => (
              <div key={h.hour} className="flex items-center justify-between bg-green-50 rounded-lg px-4 py-3">
                <span className="font-semibold text-sm text-green-800">{h.label}</span>
                <span className="font-bold text-sm text-green-600">{h.orders} {t('business.reports.orders')}</span>
              </div>
            ))}
            {busyHours.length === 0 && <p className="text-slate-300 text-xs font-semibold py-4 text-center">{t('business.reports.noData')}</p>}
          </div>
        </div>
        <div className="bg-white p-6 md:p-8 rounded-lg border border-slate-200">
          <div className="flex items-center gap-2 mb-4">
            <TrendingDown size={18} className="text-orange-500" />
            <h4 className="font-bold text-sm">{t('business.reports.slowHours')}</h4>
          </div>
          <div className="space-y-2">
            {slowHours.map((h, i) => (
              <div key={h.hour} className="flex items-center justify-between bg-orange-50 rounded-lg px-4 py-3">
                <span className="font-semibold text-sm text-orange-800">{h.label}</span>
                <span className="font-bold text-sm text-orange-600">{h.orders} {t('business.reports.orders')}</span>
              </div>
            ))}
            {slowHours.length === 0 && <p className="text-slate-300 text-xs font-semibold py-4 text-center">{t('business.reports.noData')}</p>}
          </div>
        </div>
      </div>
      ...
      {/* Top products chart */}
      <div className="bg-white p-6 md:p-10 rounded-xl border border-slate-200 shadow-sm">
        <h4 className="text-lg md:text-xl font-bold mb-1">{t('business.reports.topProducts')}</h4>
        <p className="text-slate-500 text-xs font-semibold mb-6">{t('business.reports.topProductsDesc')}</p>
        {productChart}
      </div>

      {/* Product sales by hour table */}
      <div className="bg-white p-6 md:p-10 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
        <h4 className="text-lg md:text-xl font-bold mb-1">{t('business.reports.productSalesByTime')}</h4>
        <p className="text-slate-500 text-xs font-semibold mb-6">{t('business.reports.productSalesByTimeDesc')}</p>
        <table className="w-full text-xs md:text-sm" dir="rtl">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-right py-3 px-2 font-semibold text-slate-500 text-xs">{t('business.reports.product')}</th>
              <th className="text-center py-3 px-2 font-semibold text-slate-500 text-xs">{t('business.reports.quantity')}</th>
              <th className="text-center py-3 px-2 font-semibold text-slate-500 text-xs">{t('business.reports.revenue')}</th>
              {Array.from({ length: 24 }, (_, h) => (
                <th key={h} className="text-center py-3 px-1 font-semibold text-slate-400 text-xs hidden md:table-cell">
          ...
          <tbody>
            {topProducts.map((p) => (
              <tr key={p.productId} className="border-b border-slate-200 hover:bg-slate-50/50">
                <td className="py-3 px-2 font-semibold text-slate-700">{p.name}</td>
                <td className="text-center py-3 px-2 font-bold text-cyan-600">{p.totalQty}</td>
                <td className="text-center py-3 px-2 font-semibold text-slate-600">{t('business.reports.currency')} {p.totalRevenue.toLocaleString()}</td>
                {Array.from({ length: 24 }, (_, h) => {
