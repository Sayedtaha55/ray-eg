...
const Settings: React.FC<SettingsProps> = ({ shop, onSaved, adminShopId }) => {
  ...
  const renderTabContent = (tabId: SettingsTab) => {
    switch (tabId) {
      ...
      case 'social_media':
        return <SocialMediaSettings shop={shop} onSaved={onSaved} adminShopId={adminShopId} />;
      case 'notifications':
        return (
          <div className={`space-y-6 ${isArabic ? 'text-right' : 'text-left'}`} dir={isArabic ? 'rtl' : 'ltr'}>
          ...
          {sidebarMode ? (
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              <div className="p-4">
                ...
                <div className={cn(String(activeSettingsTab) === 'social_media' ? 'block' : 'hidden')}>
                  {renderTabContent('social_media')}
                </div>
