...
const BookingSettings: React.FC<BookingSettingsProps> = ({ shop, onSaved, adminShopId }) => {
  ...
  const initial = useMemo(
    ...
    [
      pd.bookingSlotDuration,
      pd.bookingAdvanceDays,
      pd.bookingAutoConfirm,
      pd.bookingNotifyOwner,
      pd.bookingNotifyCustomer,
      pd.bookingWorkStart,
      pd.bookingWorkEnd,
      pd.bookingMaxPerSlot,
    ]
  );
  ...
  useEffect(() => {
    ...
    try {
      window.dispatchEvent(new CustomEvent('merchant-settings-section-changes', { detail: { sectionId: 'booking_settings', count: 0 } }));
    } catch {}
  }, [shop?.id, initial]);
  ...
  useEffect(() => {
    ...
    const count =
      (slotDuration !== base.bookingSlotDuration ? 1 : 0) +
      (advanceDays !== base.bookingAdvanceDays ? 1 : 0) +
      (autoConfirm !== base.bookingAutoConfirm ? 1 : 0) +
      (notifyOwner !== base.bookingNotifyOwner ? 1 : 0) +
      (notifyCustomer !== base.bookingNotifyCustomer ? 1 : 0) +
      (workStart !== base.bookingWorkStart ? 1 : 0) +
      (workEnd !== base.bookingWorkEnd ? 1 : 0) +
      (maxPerSlot !== base.bookingMaxPerSlot ? 1 : 0);

    try {
      window.dispatchEvent(new CustomEvent('merchant-settings-section-changes', { detail: { sectionId: 'booking_settings', count } }));
    } catch {}
  }, [slotDuration, advanceDays, autoConfirm, notifyOwner, notifyCustomer, workStart, workEnd, maxPerSlot]);
  ...
  const saveBookingSettings = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: t('storeSettings.saved'), description: 'تم حفظ إعدادات الحجوزات بنجاح' });
      onSaved();
      return true;
    } catch (e: any) {
      toast({
        title: t('storeSettings.error'),
        description: 'فشل حفظ إعدادات الحجوزات',
        variant: 'destructive',
      });
      return false;
    } finally {
  ...
  }, [adminShopId, onSaved, pd, slotDuration, advanceDays, autoConfirm, notifyOwner, notifyCustomer, workStart, workEnd, maxPerSlot, toast, t]);
  ...
  const Toggle = ({ value, onChange, label, description }: { value: boolean; onChange: (v: boolean) => void; label: string; description?: string }) => (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-slate-50 last:border-0">
      ...
      >
        <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-all ${value ? (isArabic ? 'right-0.5' : 'left-6') : (isArabic ? 'right-6' : 'left-0.5')}`} />
      </button>
    </div>
  );
  ...
  return (
    <div className={`space-y-6 ${isArabic ? 'text-right' : 'text-left'}`} dir={isArabic ? 'rtl' : 'ltr'}>
      <div>
        ...
        <p className="text-muted-foreground">ضبط سلوك ومواعيد الحجوزات لمتجرك</p>
      </div>
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent>
          <Toggle
            ...
            description="يتم تأكيد الحجوزات فور استلامها بدون مراجعة يدوية"
          />
        </CardContent>
      </Card>
