    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-black text-slate-400 uppercase tracking-wide">إجمالي العملاء</span>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-black text-slate-400 uppercase tracking-wide">نسبة العائدين</span>
            </div>
            <p className="text-3xl font-black text-slate-900">{analytics.retentionRate}%</p>
            <p className="text-xs text-slate-400 font-bold mt-1">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-black text-slate-400 uppercase tracking-wide">متوسط الزيارات</span>
            </div>
            <p className="text-3xl font-black text-slate-900">{analytics.avgVisitsPerCustomer}</p>
            <p className="text-xs text-slate-400 font-bold mt-1">زيارة لكل عميل</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-black text-slate-400 uppercase tracking-wide">معرضون للتوقف</span>
            </div>
            <p className="text-3xl font-black text-slate-900">{analytics.atRiskCustomers?.length || 0}</p>
            <p className="text-xs text-slate-400 font-bold mt-1">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="bg-slate-50 rounded-xl px-4 py-2 text-sm font-bold border-none outline-none cursor-pointer hover:bg-slate-100 transition-all"
        >
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-slate-600 bg-slate-50 hover:bg-slate-100 transition-all"
        >
          ...
          <table className="w-full text-right border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">{activityVocab.customerSingular}</th>
                ...
                <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">آخر زيارة</th>
                ...
                sectionCustomers.map((c: any) => {
                  ...
                  return (
                    <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="p-6 font-black text-slate-500">{c.orders || c.orderCount || 0} {t('business.customers.orders')}</td>
                      <td className="p-6">
                        <div>
                          {lastVisitDays !== null ? (
                            <span className={`text-xs font-black ${
                              isChurned ? 'text-red-500' : isAtRisk ? 'text-amber-600' : 'text-slate-400'
                            }`}>
                      ...
                      <td className="p-6 text-left">
                        <div className="flex gap-2 justify-end">
                          <button
                            ...
                            className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl font-black text-[10px] hover:bg-blue-600 hover:text-white transition-all"
                            title="عرض التفاصيل"
                          >
                            <Eye size={12} />
                          </button>
          ...
          <div className="relative ml-auto w-full max-w-lg h-full bg-white shadow-2xl overflow-y-auto">
            ...
            <div className="sticky top-0 bg-white border-b border-slate-100 p-6 flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-lg">{customerDetail?.customer?.name || selectedCustomer?.name || 'عميل'}</h3>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {customerDetail.customer?.phone && (
                    <div className="bg-slate-50 rounded-xl p-3 flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-slate-700 truncate">{customerDetail.customer.phone}</span>
                    </div>
                  )}
                  {customerDetail.customer?.email && (
                    <div className="bg-slate-50 rounded-xl p-3 flex items-center gap-2">
                      <Mail className="w-4 h-4 text-slate-400 flex-shrink-0" />
                      <span className="text-sm font-bold text-slate-700 truncate">{customerDetail.customer.email}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-blue-50 rounded-xl p-4 text-center">
                    ...
                    <p className="text-2xl font-black text-slate-900">{customerDetail.stats?.totalOrders || 0}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">طلبات</p>
                  </div>
                  <div className="bg-green-50 rounded-xl p-4 text-center">
                    <Wallet className="w-5 h-5 text-green-600 mx-auto mb-2" />
                    <p className="text-2xl font-black text-slate-900">{(customerDetail.stats?.totalSpent || 0).toLocaleString()}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">إجمالي الإنفاق</p>
                  </div>
                  <div className="bg-purple-50 rounded-xl p-4 text-center">
                    <TrendingUp className="w-5 h-5 text-purple-600 mx-auto mb-2" />
                    <p className="text-2xl font-black text-slate-900">{customerDetail.stats?.avgOrderValue || 0}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">متوسط الطلب</p>
                  </div>
                </div>
                ...
                {customerDetail.monthlySpending && customerDetail.monthlySpending.length > 0 && (
                  <div className="bg-slate-50 rounded-2xl p-4">
                    <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">الإنفاق الشهري (آخر 6 أشهر)</p>
                ...
                {customerDetail.favoriteProducts && customerDetail.favoriteProducts.length > 0 && (
                  <div>
                    <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">المنتجات المفضلة</p>
                    <div className="space-y-2">
                      {customerDetail.favoriteProducts.map((p: any, i: number) => (
                        <div key={i} className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
                          ...
                          <span className="text-xs font-black text-slate-400 bg-white px-2 py-1 rounded-lg">{p.count}x</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Order History */}
                <div>
                  <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">سجل الطلبات</p>
                      ...
                      customerDetail.orders.map((o: any, i: number) => (
                        <div key={i} className="flex items-center justify-between bg-white border border-slate-100 rounded-xl p-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-slate-700">{t('business.customers.currency')} {o.total.toLocaleString()}</p>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 transition-all"
                  >
