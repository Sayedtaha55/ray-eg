...
const ReportsTab: React.FC<Props> = ({ analytics, sales, reservations, posEnabled, onOpenCashierReports, shop }) => {
        ...
        <div className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 md:mb-12">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-2xl md:text-3xl font-black">{isArabic ? 'تقارير وإحصائيات الحجوزات' : 'Booking Reports & Statistics'}</h3>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setRange('30d')} className={`px-3 py-2 md:px-4 rounded-xl text-xs font-bold ${range === '30d' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}>{isArabic ? '٣٠ يوم' : '30D'}</button>
              <button onClick={() => setRange('6m')} className={`px-3 py-2 md:px-4 rounded-xl text-xs font-bold ${range === '6m' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}>{isArabic ? '٦ شهور' : '6M'}</button>
              <button onClick={() => setRange('12m')} className={`px-3 py-2 md:px-4 rounded-xl text-xs font-bold ${range === '12m' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600'}`}>{isArabic ? '١٢ شهر' : '12M'}</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
            <div className="bg-cyan-50 p-4 rounded-2xl border border-cyan-100">
              ...
              <div className="text-2xl font-black text-slate-900">{safeReservations.length}</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'إجمالي الحجوزات' : 'Total Bookings'}</div>
            </div>
            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
              <CheckCircle2 className="w-5 h-5 text-emerald-600 mb-2" />
              <div className="text-2xl font-black text-slate-900">{completedBookings}</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'مكتملة' : 'Completed'}</div>
            </div>
            <div className="bg-red-50 p-4 rounded-2xl border border-red-100">
              <XCircle className="w-5 h-5 text-red-400 mb-2" />
              <div className="text-2xl font-black text-slate-900">{cancelledBookings}</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'ملغاة' : 'Cancelled'}</div>
            </div>
            <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
              <DollarSign className="w-5 h-5 text-blue-500 mb-2" />
              <div className="text-xl font-black text-slate-900">{t('business.reports.currency')} {Math.round(avgBookingValue).toLocaleString()}</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'متوسط قيمة الحجز' : 'Avg Booking Value'}</div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
          <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-lg font-black">{isArabic ? 'توزيع حالات الحجوزات' : 'Booking Status Distribution'}</h4>
            </div>
            <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? 'نسبة كل حالة من إجمالي الحجوزات' : 'Breakdown of all booking statuses'}</p>
            {bookingStatusChart || (
          ...
          <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-lg font-black">{isArabic ? 'حجوزات آخر ١٤ يوم' : 'Bookings — Last 14 Days'}</h4>
            </div>
            <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? 'عدد الحجوزات اليومية' : 'Daily booking count trend'}</p>
            {bookingTrendChart || (
              <div className="py-12 text-center text-slate-300 font-bold">{isArabic ? 'لا توجد بيانات' : 'No data available'}</div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'إجمالي الإيرادات' : 'Total Revenue'}</p>
            </div>
            <span className="text-2xl md:text-3xl font-black text-slate-900">{t('business.reports.currency')} {Math.round(bookingRevenue).toLocaleString()}</span>
            {revenueGrowth !== 0 && (
              <div className="flex items-center gap-2 mt-2">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${revenueGrowth >= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50'}`}>
                  {revenueGrowth >= 0 ? '+' : ''}{Math.round(revenueGrowth)}{t('business.reports.percent')}
                </span>
                <span className="text-[10px] text-slate-400 font-bold">{isArabic ? 'مقارنة بالفترة السابقة' : 'vs previous period'}</span>
              </div>
            )}
          </div>
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'إجمالي المصروفات' : 'Total Expenses'}</p>
            </div>
            <span className="text-2xl md:text-3xl font-black text-slate-900">{t('business.reports.currency')} {Math.round(totalExpenses).toLocaleString()}</span>
          </div>
          {isBookingActivity && inventoryValue > 0 && (
            <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'قيمة المخزون' : 'Inventory Value'}</p>
              </div>
              <span className="text-2xl md:text-3xl font-black text-violet-700">{t('business.reports.currency')} {Math.round(inventoryValue).toLocaleString()}</span>
            </div>
          )}
          <div className={`bg-white p-6 md:p-8 rounded-[2.5rem] border shadow-sm ${netProfit >= 0 ? 'border-emerald-100' : 'border-red-100'}`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'صافي الربح' : 'Net Profit'}</p>
            </div>
            <span className={`text-2xl md:text-3xl font-black ${netProfit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{t('business.reports.currency')} {Math.round(netProfit).toLocaleString()}</span>
            {isBookingActivity && inventoryValue > 0 && (
              <p className="text-[10px] text-slate-400 font-bold mt-2">{isArabic ? '(الإيرادات - المصروفات - المخزون)' : '(Revenue - Expenses - Inventory)'}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'هامش الربح' : 'Profit Margin'}</p>
            </div>
            <span className="text-2xl font-black">{profitMargin.toFixed(1)}{t('business.reports.percent')}</span>
          </div>
          <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
            <div className="flex items-center gap-2 mb-2">
              <XCircle size={16} className="text-red-400" />
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'معدل الإلغاء' : 'Cancellation Rate'}</p>
            </div>
            <span className="text-2xl font-black">{safeReservations.length > 0 ? Math.round((cancelledBookings / safeReservations.length) * 100) : 0}{t('business.reports.percent')}</span>
          </div>
          <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle2 size={16} className="text-emerald-500" />
              <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{isArabic ? 'معدل الإكمال' : 'Completion Rate'}</p>
            </div>
            <span className="text-2xl font-black">{safeReservations.length > 0 ? Math.round((completedBookings / safeReservations.length) * 100) : 0}{t('business.reports.percent')}</span>
          </div>
        </div>
        ...
        {range !== '30d' && revenueVsExpensesChart && (
          <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-lg font-black">{isArabic ? 'الإيرادات مقابل المصروفات' : 'Revenue vs Expenses'}</h4>
            </div>
            <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? 'مقارنة شهرية بين الإيرادات والمصروفات والربح' : 'Monthly comparison of revenue, expenses, and profit'}</p>
        ...
        {isBookingActivity && inventoryCategoryChart && (
          <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-lg font-black">{isArabic ? 'توزيع المخزون الطبي' : 'Inventory Distribution'}</h4>
            </div>
            <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? 'توزيع الأصناف حسب التصنيف' : 'Items breakdown by category'}</p>
        ...
        {customerAnalytics && (
          <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-lg font-black">{isArabic ? 'تحليلات العملاء' : 'Customer Insights'}</h4>
            </div>
            <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? 'نظرة شاملة على سلوك العملاء' : 'Comprehensive customer behavior overview'}</p>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                <Users className="w-5 h-5 text-blue-600 mb-2" />
                <div className="text-2xl font-black text-slate-900">{customerAnalytics.totalCustomers || 0}</div>
              ...
              <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
                ...
                <div className="text-2xl font-black text-slate-900">{customerAnalytics.retentionRate || 0}%</div>
                <div className="text-xs font-bold text-slate-400">{isArabic ? 'نسبة العائدين' : 'Retention Rate'}</div>
              </div>
              <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                <TrendingUp className="w-5 h-5 text-purple-600 mb-2" />
                <div className="text-2xl font-black text-slate-900">{customerAnalytics.avgVisitsPerCustomer || 0}</div>
                <div className="text-xs font-bold text-slate-400">{isArabic ? 'متوسط الزيارات' : 'Avg Visits'}</div>
              </div>
              <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
                <AlertTriangle className="w-5 h-5 text-amber-600 mb-2" />
                <div className="text-2xl font-black text-slate-900">{customerAnalytics.atRiskCustomers?.length || 0}</div>
                <div className="text-xs font-bold text-slate-400">{isArabic ? 'معرضون للتوقف' : 'At Risk'}</div>
              </div>
            </div>

            {customerAnalytics.topCustomers && customerAnalytics.topCustomers.length > 0 && (
              <div>
                <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">{isArabic ? 'أفضل 5 عملاء' : 'Top 5 Customers'}</p>
                <div className="space-y-2">
                  {customerAnalytics.topCustomers.slice(0, 5).map((c: any, i: number) => (
                    <div key={c.id || i} className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
                      <div className="flex items-center gap-2">
                        <Crown size={14} className={i === 0 ? 'text-amber-500' : 'text-slate-400'} />
                        <span className="text-sm font-bold text-slate-700">{c.name || 'عميل'}</span>
                      </div>
                      <span className="text-xs font-black text-slate-500">{t('business.reports.currency')} {Number(c.totalSpent || 0).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="space-y-6 md:space-y-10">
      ...
      <div className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 md:mb-12">
          <h3 className="text-2xl md:text-3xl font-black">{t('business.reports.reportsAndStatistics')}</h3>
          <div className="flex gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.totalRevenue')}</p>
          </div>
          <span className="text-2xl md:text-3xl font-black text-slate-900">{t('business.reports.currency')} {Math.round(totalRevenue).toLocaleString()}</span>
          <div className="flex items-center gap-2 mt-2">
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${revenueGrowth >= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50'}`}>
              {revenueGrowth >= 0 ? '+' : ''}{Math.round(revenueGrowth)}{t('business.reports.percent')}
            </span>
            <span className="text-[10px] text-slate-400 font-bold">{t('business.reports.revenueGrowth')}</span>
          </div>
        </div>
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.totalExpenses')}</p>
          </div>
          <span className="text-2xl md:text-3xl font-black text-slate-900">{t('business.reports.currency')} {Math.round(totalExpenses).toLocaleString()}</span>
          <div className="flex items-center gap-2 mt-2">
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${expenseGrowth <= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50'}`}>
              {expenseGrowth >= 0 ? '+' : ''}{Math.round(expenseGrowth)}{t('business.reports.percent')}
            </span>
            <span className="text-[10px] text-slate-400 font-bold">{t('business.reports.expenseGrowth')}</span>
          </div>
        </div>
        <div className={`bg-white p-6 md:p-8 rounded-[2.5rem] border shadow-sm ${netProfit >= 0 ? 'border-emerald-100' : 'border-red-100'}`}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.netProfit')}</p>
          </div>
          <span className={`text-2xl md:text-3xl font-black ${netProfit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{t('business.reports.currency')} {Math.round(netProfit).toLocaleString()}</span>
          <div className="flex items-center gap-2 mt-2">
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${profitGrowth >= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50'}`}>
              {profitGrowth >= 0 ? '+' : ''}{Math.round(profitGrowth)}{t('business.reports.percent')}
            </span>
            <span className="text-[10px] text-slate-400 font-bold">{t('business.reports.profitGrowth')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
          <div className="flex items-center gap-2 mb-2">
            <BarChart3 size={16} className="text-indigo-500" />
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.profitMargin')}</p>
          </div>
          <span className="text-2xl font-black">{profitMargin.toFixed(1)}{t('business.reports.percent')}</span>
        </div>
        <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.completedBookings')}</p>
          </div>
          <span className="text-2xl font-black">{completedBookings}</span>
        </div>
        <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
          <div className="flex items-center gap-2 mb-2">
            <XCircle size={16} className="text-red-400" />
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.cancelledBookings')}</p>
          </div>
          <span className="text-2xl font-black">{cancelledBookings}</span>
        </div>
        <div className="bg-white p-5 md:p-7 rounded-[2rem] border border-slate-100">
          <div className="flex items-center gap-2 mb-2">
            <DollarSign size={16} className="text-cyan-500" />
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">{t('business.reports.avgBookingValue')}</p>
          </div>
          <span className="text-xl font-black">{t('business.reports.currency')} {Math.round(avgBookingValue).toLocaleString()}</span>
        </div>
      </div>
      ...
      {range !== '30d' && (
        <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            <BarChart3 size={18} className="text-indigo-500" />
            <h4 className="text-lg font-black">{t('business.reports.revenueVsExpenses')}</h4>
          </div>
          <p className="text-slate-400 text-xs font-bold mb-6">{t('business.reports.revenueVsExpensesDesc')}</p>
          {revenueVsExpensesChart || (
            <div className="py-12 text-center text-slate-300 font-bold">{t('business.reports.noProfitData')}</div>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
        <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h4 className="text-lg font-black">{t('business.reports.dailyTrend')}</h4>
          </div>
          <p className="text-slate-400 text-xs font-bold mb-6">{t('business.reports.dailyTrendDesc')}</p>
          {dailyTrendChart}
        </div>
        <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            <Clock size={18} className="text-purple-500" />
            <h4 className="text-lg font-black">{t('business.reports.peakHours')}</h4>
          </div>
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h4 className="text-lg font-black">{isArabic ? `أبرز ${activityVocab.productPlural}` : `Top ${activityVocab.productPlural}`}</h4>
        </div>
        <p className="text-slate-400 text-xs font-bold mb-6">{isArabic ? `أكثر ${activityVocab.productPlural} مبيعاً` : `Best selling ${activityVocab.productPlural}`}</p>
        {topProductsData.length === 0 ? (
          <div className="py-12 text-center text-slate-300 font-bold">{t('business.reports.noData')}</div>
        ) : (
                ...
                <div key={p.productId} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-black text-slate-500 ml-2 shrink-0">{p.qty} {t('business.reports.quantity')}</span>
                    </div>
                  ...
                  <span className="text-xs font-bold text-slate-400 w-20 text-left shrink-0">{t('business.reports.currency')} {p.revenue.toLocaleString()}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
            ...
            <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
              ...
              <div className="text-2xl font-black text-slate-900">{customerAnalytics.retentionRate || 0}%</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'نسبة العائدين' : 'Retention Rate'}</div>
              <div className="text-[10px] font-bold text-slate-400 mt-1">{customerAnalytics.returningCustomers || 0} {isArabic ? 'عميل عائد' : 'returning'}</div>
            </div>
            <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
              <TrendingUp className="w-5 h-5 text-purple-600 mb-2" />
              <div className="text-2xl font-black text-slate-900">{customerAnalytics.avgVisitsPerCustomer || 0}</div>
            ...
            <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
              ...
              <div className="text-2xl font-black text-slate-900">{customerAnalytics.atRiskCustomers?.length || 0}</div>
              <div className="text-xs font-bold text-slate-400">{isArabic ? 'معرضون للتوقف' : 'At Risk'}</div>
              <div className="text-[10px] font-bold text-red-400 mt-1">{customerAnalytics.churnedCustomers?.length || 0} {isArabic ? 'توقفوا' : 'churned'}</div>
            </div>
          </div>
                ...
                {customerAnalytics.topCustomers.slice(0, 5).map((c: any, i: number) => (
                  <div key={c.id || i} className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
                    <div className="flex items-center gap-2">
                      <Crown size={14} className={i === 0 ? 'text-amber-500' : 'text-slate-400'} />
                      <span className="text-sm font-bold text-slate-700">{c.name || 'عميل'}</span>
                      <span className="text-[10px] text-slate-400 font-bold">({c.orders || 0} {isArabic ? 'طلبات' : 'orders'})</span>
                    </div>
                    <span className="text-xs font-black text-slate-500">{t('business.reports.currency')} {Number(c.totalSpent || 0).toLocaleString()}</span>
                  </div>
                ))}
      ...
      {posEnabled && onOpenCashierReports && (
        <div className="bg-gradient-to-l from-cyan-50 to-indigo-50 p-6 md:p-10 rounded-[2rem] border border-cyan-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-lg font-black text-slate-800">{t('business.reports.cashierReports')}</h4>
                <p className="text-slate-500 text-xs font-bold">{t('business.reports.cashierReportsDesc')}</p>
              </div>
            </div>
            <button
              onClick={onOpenCashierReports}
              className="px-6 py-3 rounded-2xl bg-slate-900 text-white font-black text-sm hover:bg-slate-800 transition-all shadow-md whitespace-nowrap"
            >
