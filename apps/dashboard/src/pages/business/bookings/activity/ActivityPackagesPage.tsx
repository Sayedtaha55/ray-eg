...
const LABELS_EN: Record<string, string> = {
  salon_barber: 'Care Packages',
  wellness_spa: 'Packages',
};
...
const ActivityPackagesPage: React.FC<Props> = ({ activityType }) => {
  ...
  const title = LABELS[activityType] || (isEn ? 'Packages' : 'الباقات');
    ...
    <div className="space-y-5 text-right" dir={isEn ? 'ltr' : 'rtl'}>
      ...
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold mt-0.5">{packages.length} {isEn ? 'packages' : 'باقة'} • {packages.filter(p => p.isActive).length} {isEn ? 'active' : 'نشطة'}</p>
          </div>
        </div>
        <button type="button" onClick={() => setShowForm(!showForm)} title={isEn ? 'Add new package' : 'إضافة باقة جديدة'}
          ...
          <Plus size={16} /> {isEn ? 'Add Package' : 'إضافة باقة'}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">{isEn ? 'Add New Package' : 'إضافة باقة جديدة'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Package Name *' : 'اسم الباقة *'}</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder={isEn ? 'e.g. Complete Care Package' : 'مثال: باقة العناية الشاملة'}
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Original Price' : 'السعر قبل الخصم'}</label>
              <input type="number" value={form.originalPrice} onChange={e => setForm(f => ({ ...f, originalPrice: e.target.value }))}
                placeholder="600"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Duration (min)' : 'المدة (دقيقة)'}</label>
          ...
          <div>
            <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Included Services (comma-separated)' : 'الخدمات المشمولة (مفصولة بفاصلة)'}</label>
            <input type="text" value={form.services} onChange={e => setForm(f => ({ ...f, services: e.target.value }))}
              placeholder={isEn ? 'Haircut, Color, Styling' : 'قص شعر, صبغة, تسريحة'}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
          </div>
          <div className="flex gap-3 justify-end">
            ...
            <button type="button" onClick={handleAdd} disabled={!form.name.trim() || !form.price} title={isEn ? 'Save package' : 'حفظ الباقة'}
      ...
      <div className="relative">
        ...
        <input type="text" placeholder={isEn ? 'Search packages...' : 'ابحث في الباقات...'} value={search} onChange={e => setSearch(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">{search ? (isEn ? `No results for "${search}"` : `لا نتائج لـ "${search}"`) : (isEn ? 'No packages added yet' : 'لم تضف باقات بعد')}</p>
        </div>
      ) : (
          ...
          {filtered.map(pkg => (
            <div key={pkg.id} className={`bg-white rounded-[2rem] border shadow-sm p-5 space-y-3 hover:shadow-md transition-all ${pkg.isActive ? 'border-slate-100' : 'border-red-100 opacity-60'}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-400 font-bold mt-0.5">{pkg.services.length} {isEn ? 'services included' : 'خدمة مشمولة'}</div>
                  </div>
                </div>
                <button type="button" onClick={() => toggleActive(pkg.id)} title={pkg.isActive ? (isEn ? 'Disable package' : 'تعطيل الباقة') : (isEn ? 'Enable package' : 'تفعيل الباقة')}
                  ...
                  {pkg.isActive ? (isEn ? 'Active' : 'نشط') : (isEn ? 'Disabled' : 'معطل')}
                </button>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1 text-sm font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg">
                  <DollarSign size={13} /> {pkg.price} {isEn ? 'EGP' : 'ج.م'}
                </div>
                {pkg.originalPrice && (
                  <span className="text-xs text-slate-400 line-through font-bold">{pkg.originalPrice} {isEn ? 'EGP' : 'ج.م'}</span>
                )}
                <div className="flex items-center gap-1 text-xs font-bold text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg">
                  <Clock size={11} /> {pkg.durationMinutes} {isEn ? 'min' : 'د'}
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" title={isEn ? 'Edit package' : 'تعديل الباقة'} className="flex-1 py-2 rounded-xl border border-slate-200 text-xs font-black text-slate-600 hover:bg-slate-50 flex items-center justify-center gap-1.5">
                  <Edit2 size={12} /> {isEn ? 'Edit' : 'تعديل'}
                </button>
                <button type="button" onClick={() => handleDelete(pkg.id)} title={isEn ? 'Delete package' : 'حذف الباقة'}
