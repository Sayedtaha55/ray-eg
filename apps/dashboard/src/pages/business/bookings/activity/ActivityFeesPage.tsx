...
const ActivityFeesPage: React.FC<Props> = ({ activityType }) => {
  ...
  const formatAmount = (fee: Fee) => {
    if (fee.type === 'per_km') return `${fee.amount} ${isEn ? 'EGP/km' : 'ج.م/كم'}`;
    if (fee.type === 'percentage') return `${fee.amount}%`;
    return `${fee.amount} ${isEn ? 'EGP' : 'ج.م'}`;
  };
    ...
    <div className="space-y-5 text-right" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">{isEn ? 'Transfer Fees' : 'رسوم الانتقال'}</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{fees.filter(f => f.isActive).length} {isEn ? 'active of' : 'رسم نشط من'} {fees.length}</p>
          </div>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          ...
          <Plus size={16} /> {isEn ? 'Add Fee' : 'إضافة رسم'}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">{isEn ? 'Add New Fee' : 'إضافة رسم جديد'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Fee Name *' : 'اسم الرسم *'}</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder={isEn ? 'e.g. Transfer Fee' : 'مثال: رسوم الانتقال'}
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Amount *' : 'المبلغ *'}</label>
            ...
            <div>
              ...
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as Fee['type'] }))}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300">
                <option value="fixed">{isEn ? 'Fixed amount' : 'مبلغ ثابت'}</option>
                <option value="per_km">{isEn ? 'Per km' : 'لكل كيلومتر'}</option>
                <option value="percentage">{isEn ? 'Percentage' : 'نسبة مئوية'}</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Applies To' : 'يُطبق على'}</label>
              <input type="text" value={form.appliesTo} onChange={e => setForm(f => ({ ...f, appliesTo: e.target.value }))}
                placeholder={isEn ? 'e.g. All visits' : 'مثال: جميع الزيارات'}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Min Order (EGP)' : 'حد أدنى للطلب (ج.م)'}</label>
      ...
      {fees.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">{isEn ? 'No fees added yet' : 'لم تضف رسوم بعد'}</p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap text-xs font-bold">
                      ...
                      {fee.minOrder && <span className="text-slate-400">{isEn ? 'Min:' : 'حد أدنى:'} {fee.minOrder} {isEn ? 'EGP' : 'ج.م'}</span>}
                    </div>
