  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const newRule: CapacityRule = {
        ...
        period: form.period || (isEn ? 'Daily' : 'يومياً'),
    ...
    <div className="space-y-5 text-right" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">{isEn ? 'Capacity Rules' : 'قواعد السعة القصوى'}</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{rules.length} {isEn ? 'active rules' : 'قاعدة نشطة'}</p>
          </div>
        </div>
        <button type="button" onClick={() => setShowForm(!showForm)} title={isEn ? 'Add new capacity rule' : 'إضافة قاعدة سعة جديدة'}
          ...
          <Plus size={16} /> {isEn ? 'Add Rule' : 'إضافة قاعدة'}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">{isEn ? 'Add New Capacity Rule' : 'إضافة قاعدة سعة جديدة'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Rule Name *' : 'اسم القاعدة *'}</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder={isEn ? 'e.g. Main Hall Capacity' : 'مثال: سعة الصالة الرئيسية'}
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Max Capacity *' : 'الحد الأقصى *'}</label>
              <input type="number" value={form.maxCapacity} onChange={e => setForm(f => ({ ...f, maxCapacity: e.target.value }))}
                placeholder="50" min="1"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Period' : 'الفترة'}</label>
              <input type="text" value={form.period} onChange={e => setForm(f => ({ ...f, period: e.target.value }))}
                placeholder={isEn ? 'Daily / Weekly' : 'يومياً / أسبوعياً'}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button type="button" onClick={handleAdd} disabled={!form.name.trim() || !form.maxCapacity} title={isEn ? 'Save capacity rule' : 'حفظ قاعدة السعة'}
      ...
      <div className="relative">
        ...
        <input type="text" placeholder={isEn ? 'Search rules...' : 'ابحث في القواعد...'} value={search} onChange={e => setSearch(e.target.value)}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">{search ? (isEn ? `No results for "${search}"` : `لا نتائج لـ "${search}"`) : (isEn ? 'No rules added yet' : 'لم تضف قواعد بعد')}</p>
        </div>
      ) : (
            ...
            <div key={rule.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-5 space-y-3 hover:shadow-md transition-all">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-1 text-sm font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg">
                  <Users size={12} /> {rule.maxCapacity} {isEn ? 'people' : 'شخص'}
                </div>
              </div>
              {rule.notes && <p className="text-xs text-slate-400 font-bold">{rule.notes}</p>}
              <div className="flex gap-2 pt-1">
                <button type="button" title={isEn ? 'Edit rule' : 'تعديل القاعدة'} className="flex-1 py-2 rounded-xl border border-slate-200 text-xs font-black text-slate-600 hover:bg-slate-50 flex items-center justify-center gap-1.5">
                  <Edit2 size={12} /> {isEn ? 'Edit' : 'تعديل'}
                </button>
                <button type="button" onClick={() => handleDelete(rule.id)} title={isEn ? 'Delete rule' : 'حذف القاعدة'}
