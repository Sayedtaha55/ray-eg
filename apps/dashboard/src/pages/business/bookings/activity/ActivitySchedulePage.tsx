...
const STATUS_MAP_EN = {
  upcoming:  { label: 'Upcoming',  color: 'text-blue-700',    bg: 'bg-blue-50' },
  ongoing:   { label: 'Ongoing',   color: 'text-emerald-700', bg: 'bg-emerald-50' },
  completed: { label: 'Completed', color: 'text-slate-500',   bg: 'bg-slate-50' },
    ...
    <div className="space-y-5 text-right" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">{isEn ? 'Events Schedule' : 'جدول الفعاليات'}</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{events.filter(e => e.status === 'upcoming').length} {isEn ? 'upcoming events' : 'فعالية قادمة'}</p>
          </div>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          ...
          <Plus size={16} /> {isEn ? 'Add Event' : 'إضافة فعالية'}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">{isEn ? 'Add Event to Schedule' : 'إضافة فعالية للجدول'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Event Title *' : 'عنوان الفعالية *'}</label>
              <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                placeholder={isEn ? 'e.g. Opening Ceremony' : 'مثال: حفل الافتتاح'} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Date *' : 'التاريخ *'}</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Start Time' : 'وقت البداية'}</label>
              <input type="time" value={form.startTime} onChange={e => setForm(f => ({ ...f, startTime: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'End Time' : 'وقت النهاية'}</label>
              <input type="time" value={form.endTime} onChange={e => setForm(f => ({ ...f, endTime: e.target.value }))}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Venue' : 'المكان'}</label>
              <input type="text" value={form.venue} onChange={e => setForm(f => ({ ...f, venue: e.target.value }))}
                placeholder={isEn ? 'e.g. Main Hall' : 'مثال: القاعة الرئيسية'} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
          </div>
      ...
      {events.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">{isEn ? 'No events in schedule' : 'لا توجد فعاليات في الجدول'}</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-50 to-blue-50 flex flex-col items-center justify-center shrink-0">
                      <span className="text-[10px] font-bold text-cyan-600">{new Date(ev.date).toLocaleDateString(isEn ? 'en' : 'ar', { month: 'short' })}</span>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 font-bold flex-wrap">
                        ...
                        {ev.attendees != null && <span>{ev.attendees} {isEn ? 'attending' : 'حاضر'}</span>}
                      </div>
                    </div>
                  </div>
