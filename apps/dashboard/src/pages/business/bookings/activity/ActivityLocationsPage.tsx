...
const LABELS_AR: Record<string, { title: string; singular: string }> = {
  vehicle_rental:       { title: 'مواقع الاستلام والتسليم', singular: 'موقع' },
  general_appointments: { title: 'الفروع والمواقع',          singular: 'فرع' },
};
const LABELS_EN: Record<string, { title: string; singular: string }> = {
  vehicle_rental:       { title: 'Pickup & Drop-off Locations', singular: 'Location' },
  general_appointments: { title: 'Branches & Locations',        singular: 'Branch' },
};
    ...
    <div className="space-y-5 text-right" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold mt-0.5">{locations.filter(l => l.isActive).length} {lbl.singular} {isEn ? 'active' : 'نشط'}</p>
          </div>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          ...
          <Plus size={16} /> {isEn ? 'Add' : 'إضافة'} {lbl.singular}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">{isEn ? `Add New ${lbl.singular}` : `إضافة ${lbl.singular} جديد`}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Name *' : 'الاسم *'}</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder={isEn ? `e.g. Riyadh ${lbl.singular}` : `مثال: ${lbl.singular} الرياض`}
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Address *' : 'العنوان *'}</label>
              <input type="text" value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))}
                placeholder={isEn ? 'District, Street, Building' : 'الحي، الشارع، المبنى'}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">{isEn ? 'Working Hours' : 'أوقات العمل'}</label>
              <input type="text" value={form.workingHours} onChange={e => setForm(f => ({ ...f, workingHours: e.target.value }))}
                placeholder={isEn ? '8:00 AM - 10:00 PM' : '8:00 ص - 10:00 م'}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">{isEn ? 'No locations added yet' : 'لم تضف مواقع بعد'}</p>
        </div>
      ) : (
