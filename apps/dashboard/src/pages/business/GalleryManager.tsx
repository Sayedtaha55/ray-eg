...
import { 
  ...
  Eye, Edit2, Save, Camera, Sparkles, AlertCircle, Play
} from 'lucide-react';
...
const GalleryManager: React.FC<GalleryManagerProps> = ({ 
  ...
  const isVideoUrl = (url: string) => {
    return /\.(mp4|webm|mov|ogg|ogv|avi|3gp)(?:[?#]|$)/i.test(String(url || ''));
  };
  ...
  const handleAddFromUrl = () => {
    ...
    if (url && url.trim()) {
      ...
      const mediaType = isVideoUrl(trimmedUrl) ? 'VIDEO' : 'IMAGE';
      const newImage: ShopGallery = {
        id: `url_${Date.now()}`,
        shopId,
        imageUrl: trimmedUrl,
            ...
            {images.map((image, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/60 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-2">
                  <button
                    onClick={() => {
                      if (image.mediaType === 'VIDEO') {
                        ...
                        if (win) {
                          const safeUrl = String(image.imageUrl || '').replace(/"/g, '%22').replace(/</g, '%3C').replace(/>/g, '%3E');
                          win.document.write(`<html><head><title>${t('business.gallery.preview')}</title><style>body{margin:0;background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh}video{max-width:100%;max-height:100vh}</style></head><body><video src="${safeUrl}" controls autoplay playsinline style="width:100%"></video></body></html>`);
                          win.document.close();
                        }
                      } else {
                  ...
                  >
                    {image.mediaType === 'VIDEO' ? <Play size={16} /> : <Eye size={16} />}
                  </button>
