...
const HeaderFooterSection: React.FC<Props> = ({
  ...
  const BOOKING_VIS_KEYS = [
    'header', 'clinicHero', 'clinicAboutUs', 'clinicSpecialties', 'clinicDoctors',
    'clinicBookingWizard', 'clinicReviews', 'clinicWhyChooseUs', 'clinicFaq', 'clinicContact', 'clinicCustomPages',
  ] as const;
  ...
  const TOP_VIS_KEYS = isBooking ? BOOKING_VIS_KEYS : SHOP_VIS_KEYS;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-right pr-1">إظهار وإخفاء الهيدر</label>
        <div className="flex items-center justify-between p-3 rounded-2xl border border-slate-50 bg-slate-50/60 flex-row-reverse text-right">
          <span className="font-black text-xs text-slate-700">إظهار الهيدر بالكامل</span>
          <button
            type="button"
            onClick={() => setVis('header', !getVis('header', true))}
            className={`p-1.5 rounded-xl transition-all ${getVis('header', true) ? 'bg-[#00E5FF]/10 text-[#00E5FF]' : 'bg-slate-250 text-slate-400'}`}
          >
            {getVis('header', true) ? <Eye size={14} /> : <EyeOff size={14} />}
          </button>
        </div>
      </div>
      ...
      {/* ─── Header Style Customization (shop activities only) ─── */}
      {!isBooking && (
        <>
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-right pr-1">{isBooking ? 'إظهار وإخفاء أقسام العيادة' : 'إظهار وإخفاء عناصر الهيدر'}</label>
