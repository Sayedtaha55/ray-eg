...
const TypographySection: React.FC<Props> = ({ config, setConfig }) => {
  ...
  <div className="space-y-4">
    <div>
      <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block text-right">{t('business.builder.typography.headingSize')}</label>
      <div className="grid grid-cols-3 gap-2">
        {HEADING_SIZES.map((size) => (
          ...
          >
            <p className={`font-black ${size}`}>{t('business.builder.typography.sampleHeading')}</p>
          </button>
        ))}
      </div>
    </div>
    ...
    <div>
      <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block text-right">{t('business.builder.typography.textSize')}</label>
      <div className="grid grid-cols-3 gap-2">
        {TEXT_SIZES.map((size) => (
          <button
            key={size}
          ...
          >
            <p className={`font-black ${size}`}>{t('business.builder.typography.sampleText')}</p>
          </button>
        ))}
      </div>
    </div>
