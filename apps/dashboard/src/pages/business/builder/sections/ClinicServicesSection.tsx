...
const ClinicServicesSection: React.FC<Props> = ({ config, setConfig, shop }) => {
  ...
  const serviceLabel = vocab.serviceSingular;
  const servicesLabel = vocab.servicePlural;
  ...
  return (
    <div className="space-y-4 text-right" dir="rtl">
      <div className="border-r-4 border-cyan-400 pr-2">
        <h3 className="text-xs font-black text-slate-900">تخصيص {servicesLabel}</h3>
        <p className="text-[10px] font-bold text-slate-500 mt-0.5">أضف {servicesLabel} المتاحة وقم بتعديل أسمائها وأيقوناتها.</p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-3 shadow-inner">
        <span className="text-[10px] font-black text-cyan-600 block">
          {editingId ? `تعديل بيانات ${serviceLabel}` : `إضافة ${serviceLabel} جديد`}
        </span>

        <div className="space-y-2">
          <div>
            <label className="text-[9px] font-black text-slate-400 block mb-0.5">اسم {serviceLabel}</label>
            <input
              type="text"
              value={name}
      ...
      {/* Specialties List */}
      <div className="space-y-2">
        <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">{servicesLabel} الحالية ({specialtiesList.length})</span>
