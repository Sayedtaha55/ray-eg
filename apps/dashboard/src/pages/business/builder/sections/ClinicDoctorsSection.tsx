...
import { getShopBookingActivityType, getVocabulary } from '@/components/pages/business/bookings/config';
...
const ClinicDoctorsSection: React.FC<Props> = ({ config, setConfig, shop }) => {
  ...
  const bookingActivityType = getShopBookingActivityType(shop);
  const vocab = useMemo(() => getVocabulary(bookingActivityType || 'clinic'), [bookingActivityType]);
  const providerLabel = vocab.providerSingular;
  const providersLabel = vocab.providerPlural;
  ...
  return (
    <div className="space-y-4 text-right" dir="rtl">
      <div className="border-r-4 border-cyan-400 pr-2">
        <h3 className="text-xs font-black text-slate-900">إدارة {providersLabel}</h3>
        <p className="text-[10px] font-bold text-slate-500 mt-0.5">يمكنك إضافة {providersLabel} جدد، تعديل معلوماتهم، أو تحميل صور مخصصة لكل {providerLabel}.</p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 space-y-3 shadow-inner">
        <span className="text-[10px] font-black text-cyan-600 block">
          {editingId ? `تعديل بيانات ${providerLabel}` : `إضافة ${providerLabel} جديد`}
        </span>

        <div className="space-y-2">
          <div>
            <label className="text-[9px] font-black text-slate-400 block mb-0.5">اسم {providerLabel} بالكامل</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="text-[9px] font-black text-slate-400 block mb-0.5">{vocab.providerTitleLabel}</label>
            <input
              type="text"
              value={title}
          ...
          {/* Photo Upload */}
          <div>
            <label className="text-[9px] font-black text-slate-400 block mb-1">{vocab.providerImageLabel}</label>
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    if (editingId) {
                      ...
                      // For a new provider, upload using temporary id
              ...
              >
                {uploadingId === (editingId || 'new_temp') ? (
                  ...
                  <>
                    ...
                    <span className="text-[9px] font-bold text-slate-500">رفع صورة {providerLabel}</span>
                  </>
                )}
              </label>
      ...
      {/* Doctor List */}
      <div className="space-y-2">
        <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">{providersLabel} الحاليين ({doctorsList.length})</span>
