...
import { ChevronLeft, ChevronRight, Rows3, ArrowDown, ArrowUp } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={addRow}
          className="px-3 py-1.5 rounded-xl bg-slate-900 text-white font-bold text-xs hover:bg-slate-800 transition-all"
        >
          + صف جديد
        </button>
      </div>
          ...
          <div key={row.id} className="p-3 rounded-xl border border-slate-200 bg-white space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                <button type="button" onClick={() => removeRow(index)} className="text-red-500 font-bold text-xs hover:text-red-600 px-2">حذف</button>
              </div>
            </div>

            {/* عدد المنتجات - input رقمي حر */}
            ...
            {/* نوع العرض: carousel / grid */}
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => updateRow(index, 'rowMode', 'carousel')}
                className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1 ${String(row.rowMode || 'grid') === 'carousel' ? 'border-[#00E5FF] bg-slate-50' : 'border-slate-100'}`}
              >
              ...
              <button
                ...
                className={`p-2 rounded-xl border text-xs font-bold ${String(row.rowMode || 'grid') === 'grid' ? 'border-[#00E5FF] bg-slate-50' : 'border-slate-100'}`}
              >
                شبكة
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1 ${String(row.layoutDirection || 'rtl') === 'rtl' ? 'border-[#00E5FF] bg-slate-50' : 'border-slate-100'}`}
              >
              ...
              <button
                ...
                className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-center gap-1 ${String(row.layoutDirection || 'rtl') === 'ltr' ? 'border-[#00E5FF] bg-slate-50' : 'border-slate-100'}`}
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 block text-right">من تاريخ</label>
                <input
                  type="datetime-local"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 block text-right">إلى تاريخ</label>
                <input
                  type="datetime-local"
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="flex items-center justify-between px-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700">
                إخفاء المخزون
                <input
                  type="checkbox"
