      ...
      <div className="w-full h-full rounded-lg md:rounded-[1.4rem] bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:border-[#BD00FF] transition-all group overflow-hidden relative">
        <div className="absolute inset-0">
          ...
          {showConfigureIcon && typeof onConfigure === 'function' ? (
            <button
              type="button"
              aria-label={t('business.pos.productCard.configureAria', 'Configure')}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-1.5 md:p-3 text-right">
            ...
            <div className="flex items-center justify-between flex-row-reverse">
              <span className="text-[#00E5FF] font-black text-[9px] md:text-sm">
                {t('business.pos.egp', 'ج.م')} {getProductEffectivePrice(product).toFixed(0)}
              </span>
              {isProductTrackStockEnabled(product) && (
                <span className={`text-[8px] md:text-[10px] font-bold px-1.5 py-0.5 rounded-md ${isOutOfStock ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-white/60'}`}>
                  {isOutOfStock ? t('business.pos.productCard.outOfStock', 'نفذ') : t('business.pos.productCard.stockShort', `${stock}`)}
                </span>
              )}
            </div>
          </div>
