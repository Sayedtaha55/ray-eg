...
const PromotionsTab: React.FC<Props> = ({ shopId, offers = [], onDelete, onCreateOffer }) => {
  const t = useT();
  return (
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-sm">
      <div className="flex items-start md:items-center justify-between mb-10 flex-row-reverse gap-4">
        <h3 className="text-3xl font-black">{t('business.promotions.title')}</h3>
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 flex-row-reverse w-full md:w-auto">
          <button onClick={onCreateOffer} className="px-4 md:px-6 py-2 md:py-3 bg-slate-900 text-white rounded-2xl font-black text-[11px] md:text-xs flex items-center justify-center gap-2 hover:bg-black transition-all">
            <Tag size={14} className="md:hidden" /><Tag size={16} className="hidden md:block" />{t('business.promotions.createNewOffer')}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {offers.length === 0 ? (
          <div className="col-span-full py-32 text-center border-2 border-dashed border-slate-100 rounded-[3rem] text-slate-300 font-bold">{t('business.promotions.noActiveOffers')}</div>
        ) : (
          offers.map((offer: any) => (
            <div key={offer.id} className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col gap-6 group hover:shadow-xl transition-all">
              <div className="relative aspect-video rounded-3xl overflow-hidden shadow-sm">
                {offer.imageUrl && <img src={offer.imageUrl} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform" loading="lazy" />}
              ...
              <div className="flex gap-2">
                <button className="flex-1 py-4 bg-white border border-slate-100 rounded-2xl font-black text-xs text-slate-400">{t('business.promotions.editDesign')}</button>
                <button onClick={() => onDelete?.(offer.id)} className="flex-1 py-4 bg-red-50 text-red-500 rounded-2xl font-black text-xs hover:bg-red-500 hover:text-white transition-all">{t('business.promotions.stopOffer')}</button>
              </div>
            </div>
          ))
        )}
