...
const CanvasArea: React.FC<CanvasAreaProps> = ({
  mapImageUrl, hotspots, selectedId, addingMode, loading, imageUploading,
  onCanvasClick, onHotspotClick, canvasRef, fileInputRef,
}) => {
  const t = useT();
    ...
    return (
      <div className="relative bg-black flex items-center justify-center min-h-[400px]">
        <div className="text-white flex items-center gap-3">
          ...
          <span className="font-black text-sm">{t('imageMap.loading', 'جاري التحميل...')}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="relative bg-black flex items-center justify-center min-h-[400px]">
        <div className="text-white text-center space-y-4 p-8">
          <div className="font-black text-xl">{t('imageMap.noImageYet', 'لا توجد صورة خريطة بعد')}</div>
          <p className="text-slate-400 text-sm font-bold">{t('imageMap.uploadHint', 'ارفع صورة للمحل لتبدأ بتحديد المنتجات عليها')}</p>
          <button
            ...
            type="button"
          >
            {t('imageMap.uploadMapImage', 'رفع صورة الخريطة')}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative bg-black flex items-center justify-center overflow-hidden" style={{ contentVisibility: 'auto' }}>
      ...
      >
        ...
        />
        {hotspots.map((h) => (
          <HotspotMarker key={h.id} id={h.id} x={h.x} y={h.y} isSelected={selectedId === h.id} onClick={onHotspotClick} />
        ))}
      </div>
    </div>
  );
