...
const POSCart: React.FC<POSCartProps> = ({
    ...
    <>
      <div className="p-4 md:p-8 border-b border-slate-100 bg-white sticky top-0">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <div className="flex items-center gap-3">
            <div className="bg-[#BD00FF]/10 p-2.5 rounded-xl"><ShoppingCart className="w-6 h-6 text-[#BD00FF]" /></div>
            <h2 className="text-xl md:text-2xl font-black">{t('business.pos.cart.title', 'السلة')}</h2>
          </div>
          <span className="bg-slate-100 px-4 py-1.5 rounded-full text-xs font-black">{t('business.pos.cart.itemsCount', '{{count}} صنف').replace('{{count}}', String(cart.length))}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 no-scrollbar">
        <AnimatePresence mode="popLayout">
          {cart.map((item) => (
            <MotionDiv layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} key={item.id} className="bg-white border border-slate-100 p-3 md:p-4 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
              <div className="flex justify-between items-start mb-2">
                ...
                <button onClick={() => removeFromCart(item.id)} className="p-1.5 text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
              </div>
              <div className="flex items-center justify-between bg-slate-50 p-1.5 rounded-xl">
                <div className="flex items-center gap-2">
                  <button onClick={() => updateQuantity(item.id, 1)} className="w-9 h-9 md:w-10 md:h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center hover:border-[#BD00FF] hover:text-[#BD00FF] transition-all active:scale-90"><Plus size={18} /></button>
                  <span className="font-black text-base md:text-lg w-6 text-center">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, -1)} className="w-9 h-9 md:w-10 md:h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center hover:border-red-500 hover:text-red-500 transition-all active:scale-90"><Minus size={18} /></button>
                </div>
      ...
      <div className="p-3 md:p-8 bg-white border-t border-slate-100 space-y-3">
        <div className="space-y-2">
          ...
          {vatRatePct > 0 && (
            <div className="flex justify-between text-slate-500 font-bold text-sm md:text-base">
              <span>{t('business.pos.cart.vat', 'ضريبة {{pct}}%').replace('{{pct}}', String(vatRatePct))}</span>
  ...
  if (variant === 'mobile') {
    ...
    return (
      <>
            ...
            <div className="mx-auto max-w-[1400px] rounded-[1.6rem] bg-slate-900 text-white shadow-2xl flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-white/70 font-bold">{t('business.pos.cart.pieces', '{{count}} قطعة').replace('{{count}}', String(itemsCount))}</div>
                </div>
              </div>
              <div className="text-left">
        ...
        <AnimatePresence>{mobileOpen && <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[130] md:hidden bg-black/40" onClick={() => setMobileOpen(false)} />}</AnimatePresence>
        <AnimatePresence>
          {mobileOpen && (
            <MotionDiv key="pos-cart-sheet" initial={{ y: sheetYClosed }} animate={{ y: 0 }} exit={{ y: sheetYClosed }} transition={{ type: 'spring', stiffness: 260, damping: 28 }} drag="y" dragConstraints={{ top: 0, bottom: sheetYClosed }} dragElastic={0.12} onDragEnd={(_: any, info: any) => { if (info.offset.y > 160 || info.velocity.y > 900) setMobileOpen(false); }} className="fixed bottom-0 left-0 right-0 z-[140] md:hidden bg-white rounded-t-[2rem] shadow-2xl flex flex-col h-[92vh]">
  ...
  return <div className="hidden md:flex w-full md:w-[450px] bg-white border-r border-slate-100 flex-col shadow-2xl relative z-50">{content}</div>;
};
