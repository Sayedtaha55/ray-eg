...
export default function OffersPage() {
  ...
  useEffect(() => {
    ...
    const loadFirst = async () => {
      ...
      setHasMore(true);
      try {
        const data = await clientFetch<unknown>('/api/v1/offers?take=16&skip=0');
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const next = await clientFetch<unknown>(`/api/v1/offers?take=${take}&skip=${skip}`);
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 md:py-12 relative">
        <div className="flex flex-col items-center text-center mb-10 md:mb-16">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-7xl font-black tracking-tighter">{t('offers.title', 'Offers')}</h1>
          </div>
          <p className="text-slate-400 font-bold">{t('offers.subtitle', 'Latest active offers — progressive loading to reduce load')}</p>
        </div>
              ...
              <div className="px-3 text-right">
                ...
                <div className="flex items-center justify-between flex-row-reverse">
                  <div className="text-right">
                    <p className="text-slate-300 line-through text-xs font-bold">{t('common.currency', 'EGP')} {Number(offer.oldPrice || 0).toLocaleString(activeLocale === 'ar' ? 'ar-EG' : 'en-US')}</p>
                    <p className="text-2xl md:text-3xl font-black text-[#BD00FF] tracking-tighter">{t('common.currency', 'EGP')} {Number(offer.newPrice || 0).toLocaleString(activeLocale === 'ar' ? 'ar-EG' : 'en-US')}</p>
                  </div>
