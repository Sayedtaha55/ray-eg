...
export default function MapListingDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await clientFetch<Listing>(`/api/v1/map/listings/${id}`);
        setListing(data);
      } catch (err: any) {
        setError(err?.message || t('map.listingNotFound', 'Business not found'));
      } finally {
        setLoading(false);
      }
    })();
  ...
  if (loading) {
    return (
      <div dir={dir} className="max-w-[1400px] mx-auto px-4 md:px-6 py-6 md:py-10">
  ...
  if (error || !listing) {
    return (
      <div dir={dir} className="max-w-[1400px] mx-auto px-4 md:px-6 py-6 md:py-10 text-right">
        <div className="bg-white rounded-[2rem] border border-slate-200 p-8 text-center">
          <p className="text-slate-500 text-lg">{error || t('map.listingNotFound', 'Business not found')}</p>
          <Link href={`${prefix}/map`} className="inline-block mt-4 px-5 py-3 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all">
            {t('map.backToMap', 'Back to Map')}
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div dir={dir} className="max-w-[1400px] mx-auto px-4 md:px-6 py-6 md:py-10 text-right">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-400 font-bold mb-6">
        <Link href={`${prefix}/map`} className="hover:text-slate-700 transition-colors">{t('map.title', 'Map')}</Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Cover */}
          {listing.coverUrl && (
            <div className="rounded-[2rem] overflow-hidden h-48 md:h-64 relative">
              <Image src={listing.coverUrl} alt={listing.title} fill sizes="(max-width: 1024px) 100vw, 66vw" className="object-cover" />
            </div>
          )}
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 p-6 md:p-8">
            <div className="flex items-start gap-4">
              {listing.logoUrl ? (
                <Image src={listing.logoUrl} alt="" width={64} height={64} className="w-16 h-16 rounded-2xl object-cover shrink-0" />
              ) : (
          ...
          {listing.branches && listing.branches.length > 0 && (
            <div className="bg-white rounded-[2rem] border border-slate-200 p-6 md:p-8">
              <h2 className="text-lg font-black text-slate-900 mb-4">{t('map.branches', 'Branches')}</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 p-6 space-y-4">
            <h2 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">{t('map.contactInfo', 'Contact Info')}</h2>

            {listing.phone && (
              <a href={`tel:${listing.phone}`} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors min-w-0">
                <Phone size={18} className="text-slate-500 shrink-0" />
                <span dir="ltr" className="text-sm font-bold text-slate-700 truncate flex-1 min-w-0">{listing.phone}</span>
              </a>
            )}

            {listing.whatsapp && (
              <a href={`https://wa.me/${listing.whatsapp.replace(/^0/, '20')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-green-50 rounded-xl hover:bg-green-100 transition-colors min-w-0">
                <MessageCircle size={18} className="text-green-600 shrink-0" />
                <span dir="ltr" className="text-sm font-bold text-green-700 truncate flex-1 min-w-0">{listing.whatsapp}</span>
              </a>
            )}

            {listing.websiteUrl && (
              <a href={listing.websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors min-w-0">
                <Globe size={18} className="text-blue-600 shrink-0" />
                <span className="text-sm font-bold text-blue-700 truncate flex-1 min-w-0">{listing.websiteUrl.replace(/^https?:\/\//, '')}</span>
                <ExternalLink size={14} className="text-blue-400 shrink-0" />
              </a>
            )}

            {!listing.phone && !listing.whatsapp && !listing.websiteUrl && (
              <p className="text-sm text-slate-400">{t('map.noContact', 'No contact info')}</p>
            )}
          </div>
