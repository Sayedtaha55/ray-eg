...
import { AlertCircle, Eye, EyeOff, Loader2, ShieldCheck, UserPlus } from 'lucide-react';
import { useLocale } from '@/i18n/LocaleProvider';
import { useT } from '@/i18n/useT';
import { persistPortalSession, portalLogin } from '@/lib/auth/portal';
...
export default function PortalLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msg || t('portal.loginFailed', 'Login failed.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-20 flex items-center justify-center min-h-[80vh]" dir={dir}>
      <div className={`w-full max-w-xl bg-white border border-slate-100 p-8 md:p-16 rounded-[3.5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.05)] ${dir === 'rtl' ? 'text-right' : 'text-left'} text-slate-900`}>
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <h1 className="text-4xl font-black tracking-tighter mb-4">{t('portal.loginTitle', 'Portal Login')}</h1>
          <p className="text-slate-400 font-bold text-sm">{t('portal.loginSubtitle', 'Manage your map listing')}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button type="submit" disabled={loading} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black text-xl hover:bg-black transition-all shadow-2xl flex items-center justify-center gap-3 disabled:opacity-70">
            ...
            {loading ? t('auth.verifying', 'Verifying...') : t('portal.loginCta', 'Login')}
          </button>
        </form>

        <div className="mt-12 pt-8 border-t border-slate-50 space-y-4">
          <p className="text-center text-slate-400 font-bold text-xs mb-4">{t('portal.noAccount', "Don't have a portal account?")}</p>
          <div className="grid grid-cols-1 gap-4">
            <Link href={`/${locale}/portal/signup`} className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-slate-100 hover:bg-slate-50 transition-all text-slate-900">
