    ...
    try {
      ...
      try {
        ...
        if (cachedShop) {
          ...
          // Note: In high-performance path, we assume cached shop data is already filtered.
          // We only perform a cheap in-memory check for isImageMapCategory as a safeguard
          // to prevent potential stale data regressions if cache was populated by older code.
