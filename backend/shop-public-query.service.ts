    ...
    try {
      ...
      try {
        ...
        if (cachedShop) {
          ...
          // Performance optimization: data stored in Redis is already pre-filtered for image-map hotspots.
          // We double check isImageMapCategory here as a cheap (non-DB) safeguard for any stale cache entries.
          if (normalizedCached && Array.isArray((normalizedCached as any).products)) {
            (normalizedCached as any).products = (normalizedCached as any).products.filter(
              (p: any) => !this.isImageMapCategory((p as any)?.category),
            );
          }
