    ...
    try {
      ...
      try {
        ...
        if (cachedShop) {
          ...
          // Note: Products are already pre-filtered before caching in the DB-fallback path.
          // Zero-DB cache hits are safe because ShopImageMapService invalidates this cache on hotspot changes.
          if (normalizedCached && Array.isArray((normalizedCached as any).products)) {
            ...
            (normalizedCached as any).products = deduped.filter((p: any) => {
              // Safety fallback: ensure no stale image-map categories leaked into cache
              return !this.isImageMapCategory((p as any)?.category);
            });
          }
      ...
      if (normalizedShop && Array.isArray((normalizedShop as any).products)) {
        ...
        // Concurrent fetching of linked IDs and label keys to minimize latency during cache misses
