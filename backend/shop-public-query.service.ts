    ...
    try {
      ...
      try {
        ...
        if (cachedShop) {
          ...
          // Cheap safeguard for zero-DB cache hit path:
          // If for some reason the cached products aren't pre-filtered,
          // we at least filter by category in-memory which is O(N) and fast.
          if (normalizedCached && Array.isArray((normalizedCached as any).products)) {
            (normalizedCached as any).products = (normalizedCached as any).products.filter(
              (p: any) => !this.isImageMapCategory(p?.category)
            );
          }
