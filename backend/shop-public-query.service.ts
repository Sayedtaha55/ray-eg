...
export class ShopPublicQueryService {
  ...
  async getShopBySlug(slug: string) {
    ...
    try {
      ...
      if (normalizedShop) {
        try {
          // IMPORTANT: The normalizedShop is already filtered here (hotspots, categories, etc.)
          // We cache the filtered version to avoid redundant DB queries on cache hits.
