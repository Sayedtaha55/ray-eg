    ...
    try {
      ...
      try {
        ...
        if (cachedShop) {
          ...
          // High-performance cache hit path:
          // The cached shop data is already pre-filtered before being stored in Redis (see cache miss path below).
          // This eliminates 2 redundant database queries for hotspots.
      ...
      if (normalizedShop && Array.isArray((normalizedShop as any).products)) {
        ...
        // Parallelize hotspot lookups to reduce latency on cache misses
