...
export class ShopImageMapService {
  ...
  async activateMap(shopId: string, mapId: string, ctx: { role?: any; shopId?: any }) {
    ...
    try {
      ...
      // Clear product list caches for this shop
      await this.redis.invalidatePattern(`*products:*${sid}*`);
      // Clear global active products list cache
      await this.redis.invalidatePattern(`*products:all:*`);

      // Clear individual product caches that might be affected by hotspot changes
      // We don't know exactly which products changed, so we clear by shopId pattern
  ...
  async saveLayout(shopId: string, mapId: string, payload: any, ctx: { role?: any; shopId?: any }) {
    ...
    try {
      ...
      // Clear individual product caches to ensure consistency
      await this.redis.invalidatePattern(`product:*`);
    } catch {
    }
