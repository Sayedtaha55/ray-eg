...
export class CustomersController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('merchant', 'admin')
  async getCustomerAnalytics(@Param('shopId') shopId: string, @Request() req: any) {
    const role = String(req.user?.role || '').toUpperCase();
    const shopIdFromToken = String(req.user?.shopId || '').trim();
    ...
    try {
      return await this.customersService.getCustomerAnalytics(targetShopId);
    } catch (err: any) {
      this.logger.error(
        `getCustomerAnalytics failed (shopId=${targetShopId})`,
        err?.stack || String(err),
      );
      throw new InternalServerErrorException('حدث خطأ أثناء تحميل تحليلات العملاء');
    }
  }

  @Get('shop/:shopId/:customerId/detail')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('merchant', 'admin')
  async getCustomerDetail(@Param('shopId') shopId: string, @Param('customerId') customerId: string, @Request() req: any) {
    const role = String(req.user?.role || '').toUpperCase();
    const shopIdFromToken = String(req.user?.shopId || '').trim();
    ...
    try {
      return await this.customersService.getCustomerDetail(targetShopId, customerId);
    } catch (err: any) {
      this.logger.error(
        `getCustomerDetail failed (shopId=${targetShopId}, customerId=${customerId})`,
        err?.stack || String(err),
      );
      throw new InternalServerErrorException('حدث خطأ أثناء تحميل تفاصيل العميل');
    }
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('merchant', 'admin')
  async updateCustomerStatus(@Param('customerId') customerId: string, @Body() body: AnyDto, @Request() req: any) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('merchant', 'admin')
  async sendPromotion(@Body() body: AnyDto, @Request() req: any) {
    const role = String(req.user?.role || '').toUpperCase();
    const shopIdFromToken = String(req.user?.shopId || '').trim();
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('merchant', 'admin')
  async convert(@Body() body: AnyDto, @Request() req: any) {
    const role = String(req.user?.role || '').toUpperCase();
    const shopIdFromToken = String(req.user?.shopId || '').trim();
    const shopId = String(body?.shopId || '').trim();
