...
export class ShopImageMapService {
  ...
  async activateMap(shopId: string, mapId: string, ctx: { role?: any; shopId?: any }) {
    ...
    // Eagerly invalidate shop cache to maintain consistency for Zero-DB cache hits in getShopBySlug.
    try {
  ...
  async saveLayout(shopId: string, mapId: string, payload: any, ctx: { role?: any; shopId?: any }) {
    ...
    // Eagerly invalidate shop cache to maintain consistency for Zero-DB cache hits in getShopBySlug.
    // Invalidation happens outside transaction to avoid race conditions with stale reads.
    try {
      const shop = await this.prisma.shop.findUnique({ where: { id: sid }, select: { id: true, slug: true } });
      if (shop) {
        await this.redis.invalidateShopCache(shop.id, shop.slug);
