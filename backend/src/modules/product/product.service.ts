...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // PERFORMANCE: Parallelize independent hotspot visibility lookups to minimize latency.
    // We use Promise.allSettled to ensure transient errors in secondary checks don't block primary data.
    try {
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize primary Prisma query with independent hotspot metadata lookups
    let products: any[];
    let linkedIds: Set<string>;
    let labelKeys: Set<string>;

    try {
      const [productsRaw, linkedIdsRes, labelKeysRes] = await Promise.all([
      ...
      linkedIds = linkedIdsRes;
      labelKeys = labelKeysRes;
    } catch (e) {
    ...
    // This ensures subsequent cache hits require 0 database lookups for visibility filtering.
  ...
  async listByShopForManage(
    ...
    try {
      // PERFORMANCE: Parallelize primary Prisma query with hotspot metadata lookups to improve dashboard load times.
      const [products, linkedIds, labelKeys] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            ...
            ...(includeImageMap
              ? {
                  NOT: [{ category: '__DUPLICATE__AUTO__' }],
                }
              : {
                  NOT: [
                    { category: '__IMAGE_MAP__' },
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      const [productsRaw, linkedIdsRes] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
            NOT: [
