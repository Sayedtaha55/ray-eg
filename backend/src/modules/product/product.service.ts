...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // PERFORMANCE: Parallelize visibility checks
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize product query with visibility metadata queries
    ...
    // PERFORMANCE: Zero-DB Cache Hit pattern - filter BEFORE caching
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize product query with linked IDs query
    const [productsRaw, linkedIds] = await Promise.all([
      (async () => {
        try {
          return await (this.prisma.product as any).findMany({
            where: {
