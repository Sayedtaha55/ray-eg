...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // PERFORMANCE: Parallelize independent visibility checks to minimize latency
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize core data fetch with visibility metadata fetching
    let products: any[];
    let linkedIds: Set<string>;
    let labelKeys: Set<string>;

    try {
      [products, linkedIds, labelKeys] = await Promise.all([
      ...
      console.error('[ProductService.listByShop] Concurrent fetching failed', { shopId, error: e });
      throw this.mapDbErrorToBadRequest(e);
    }

    const deduped = this.dedupeById(products);

    // PERFORMANCE: Implement Zero-DB Cache Hit pattern.
    // Filter by visibility BEFORE caching to ensure 100% cache hits on subsequent requests.
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    let linkedIds: Set<string>;
    try {
      [products, linkedIds] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
            NOT: [
      ...
      // eslint-disable-next-line no-console
      console.error('[ProductService.listAllActive] Concurrent fetching failed', { error: e });
      throw this.mapDbErrorToBadRequest(e);
    }
  ...
  async create(input: {
    ...
    try {
      ...
      // PERFORMANCE: Surgical cache invalidation
      await Promise.all([
        this.redis.invalidatePattern(`products:shop:*${input.shopId}*`),
  ...
  async importDrafts(
    ...
    try {
      // PERFORMANCE: Surgical cache invalidation
      await Promise.all([
        this.redis.invalidatePattern(`products:shop:*${shopId}*`),
        this.redis.invalidatePattern('products:all:*'),
      ]);
    } catch {
    }
  ...
  async updateStock(productId: string, stock: number, actor: { role: string; shopId?: string }) {
    ...
    try {
      ...
      // PERFORMANCE: Surgical cache invalidation
      await Promise.all([
        this.redis.invalidatePattern(`products:shop:*${existing.shopId}*`),
        this.redis.invalidatePattern('products:all:*'),
      ]);
    } catch {
    }
