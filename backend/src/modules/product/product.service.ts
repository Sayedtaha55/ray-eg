...
export class ProductService {
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Parallelize products fetch with hotspot metadata to achieve 0-DB-query cache hits
      const [prodData, lIds, lKeys] = await Promise.all([
    ...
    // Apply visibility filters BEFORE caching (Zero-DB Cache Hit pattern)
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Parallelize products fetch with hotspot metadata
      const [prodData, lIds] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
            NOT: [
