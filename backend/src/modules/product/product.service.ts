...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    const [product, linkedIds, labelKeys] = await Promise.all([
      (this.prisma.product as any).findUnique({
        where: { id },
        select: {
          id: true,
          name: true,
      ...
      // We don't have shopId yet, so we'll fetch linkedIds globally or handle it after product fetch.
      // Actually, fetching it concurrently with the product ID is only possible if we don't need shopId.
      // But getLinkedImageMapProductIds takes an optional shopId.
      // If we don't provide it, it fetches all linked IDs.
      // Let's refine this to be more efficient.
      this.getLinkedImageMapProductIds().catch(() => new Set<string>()),
      this.getActiveImageMapHotspotLabelKeys().catch(() => new Set<string>()),
    ]);
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      ...
      products = results[0];
      linkedIds = results[1] as Set<string>;
      labelKeys = results[2] as Set<string>;
    } catch (e) {
