...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    try {
      const [linkedIdsRes, labelKeysRes] = await Promise.allSettled([
        this.getLinkedImageMapProductIds(sid),
        this.getActiveImageMapHotspotLabelKeys(sid),
      ]);

      const linkedIds = linkedIdsRes.status === 'fulfilled' ? linkedIdsRes.value : new Set<string>();
      const labelKeys = labelKeysRes.status === 'fulfilled' ? labelKeysRes.value : new Set<string>();
