...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    try {
      // PERFORMANCE: Parallelize hotspot metadata checks to reduce tail latency on cache miss
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize database queries for products and hotspot metadata to reduce latency
    ...
    // PERFORMANCE: Eliminate Zero-DB Cache Hit anti-pattern by pre-filtering before caching.
    // This ensures cache hits don't require subsequent database queries for filtering.
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Pre-filter products before caching to ensure cache hits follow the Zero-DB path
