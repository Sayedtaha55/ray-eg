...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // PERFORMANCE: Parallelize independent visibility lookups to reduce latency.
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Parallelize core product fetch and visibility metadata lookups
      const [productsRaw, linkedIdsResult, labelKeysResult] = await Promise.all([
      ...
      linkedIds = linkedIdsResult;
      labelKeys = labelKeysResult;
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error('[ProductService.listByShop] DB query failed', { shopId, error: e });
    ...
    // This ensures cache hits require no subsequent database lookups for visibility filtering.
  ...
  async listByShopForManage(
    ...
    try {
      // PERFORMANCE: Parallelize products and hotspot metadata fetch to minimize management dashboard latency
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    let linkedIds: Set<string>;
    try {
      // PERFORMANCE: Parallelize core product fetch and linked image map ID lookup
      const [productsRaw, linkedIdsResult] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
            NOT: [
      ...
      // eslint-disable-next-line no-console
      console.error('[ProductService.listAllActive] DB query failed', { error: e });
      throw this.mapDbErrorToBadRequest(e);
    }
