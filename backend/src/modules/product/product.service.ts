...
export class ProductService {
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      const [productsRaw, linkedIdsRaw, labelKeysRaw] = await Promise.all([
      ...
      linkedIds = linkedIdsRaw;
      labelKeys = labelKeysRaw;
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error('[ProductService.listByShop] Prisma / metadata parallel query failed', { shopId, error: e });
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      const [productsRaw, linkedIdsRaw] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
            NOT: [
      ...
      // eslint-disable-next-line no-console
      console.error('[ProductService.listAllActive] Prisma / metadata parallel query failed', { error: e });
      throw this.mapDbErrorToBadRequest(e);
    }
