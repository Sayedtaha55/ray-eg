...
export class ProductService {
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize core product query with visibility queries to minimize latency on cache miss
    ...
    // PERFORMANCE: Zero-DB Cache Hit pattern. Filter products against hotspot data BEFORE caching.
    // This ensures that subsequent cache hits return only public-facing products without redundant DB queries.
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Parallelize core product query with linked hotspots query
