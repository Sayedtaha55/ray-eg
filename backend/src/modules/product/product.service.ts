...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    try {
      // PERFORMANCE: Parallelize independent visibility checks to reduce latency
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    // PERFORMANCE: Zero-DB Cache Hit pattern - filter by visibility BEFORE caching
    // This ensures that subsequent cache hits return a fully filtered result without additional DB lookups.
  ...
  async listByShopForManage(
    ...
    try {
      // PERFORMANCE: Parallelize independent database queries to reduce latency
