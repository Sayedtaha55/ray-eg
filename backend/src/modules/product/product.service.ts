...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    try {
      // PERFORMANCE: Parallelize independent hotspot metadata lookups to reduce tail latency on cache miss.
      ...
      // Swallowing errors for SQL/API checks to maintain 'fail-open' behavior (show product) if metadata fetches fail.
      // EXCEPT for actual NotFoundException which we explicitly threw above.
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      ...
      // PERFORMANCE: Zero-DB Cache Hit path. We trust pre-filtered products in cache.
      if (cached) return cached;
    } catch {
    }

    try {
      // PERFORMANCE: Parallelize independent database queries to reduce tail latency on cache miss.
      const [productsRaw, linkedIds, labelKeys] = await Promise.all([
      ...
      const deduped = this.dedupeById(productsRaw);

      // PERFORMANCE: Filter hotspot-linked items BEFORE caching.
      // This ensures cache hits follow the Zero-DB path and don't require subsequent metadata lookups.
      ...
      console.error('[ProductService.listByShop] Error', { shopId, error: e });
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      const cached = await this.redis.get<any[]>(cacheKey);
      // PERFORMANCE: Zero-DB Cache Hit path.
      if (cached) return cached;
    } catch {
    }
    ...
    try {
      // PERFORMANCE: Parallelize independent database queries to reduce tail latency on cache miss.
      // We fetch hotspot data globally (no shopId) to filter products correctly.
      const [productsRaw, linkedIds, labelKeys] = await Promise.all([
        (this.prisma.product as any).findMany({
          where: {
            isActive: true,
      ...
      // eslint-disable-next-line no-console
      console.error('[ProductService.listAllActive] Error', { error: e });
      throw this.mapDbErrorToBadRequest(e);
    }
  }
