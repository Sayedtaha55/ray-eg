...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // Parallelize metadata lookups to minimize latency
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Achieve Zero-DB cache hits by parallelizing metadata lookups
      // and filtering products BEFORE caching the result in Redis.
    ...
    try {
      // Cache the filtered results to ensure subsequent hits need 0 DB queries.
  ...
  async listByShopForManage(
    ...
    try {
      // Parallelize lookups for management dashboard speed
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Achieve Zero-DB cache hits by caching the filtered list.
