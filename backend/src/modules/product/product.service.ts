...
export class ProductService {
  ...
  async getById(id: string) {
    ...
    // PERFORMANCE: Parallelize hotspot visibility lookups to minimize latency on cache miss
    ...
    try {
      // PERFORMANCE: Cache the finalized visible product (Zero-DB Cache Hit pattern)
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Parallelize core product fetch and hotspot visibility data to minimize latency
    ...
    // PERFORMANCE: Filter before caching (Zero-DB Cache Hit pattern)
    // Ensures correctness (cache hits are already filtered) and speed (0 DB queries on cache hit)
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      // PERFORMANCE: Parallelize global product fetch and linked image map IDs to minimize latency
    ...
    // PERFORMANCE: Filter before caching (Zero-DB Cache Hit pattern)
    // Eliminates the need for getLinkedImageMapProductIds calls on cache hits
