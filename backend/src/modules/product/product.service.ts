...
export class ProductService {
  ...
  async listByShop(shopId: string, paging?: { page?: number; limit?: number }) {
    ...
    try {
      ...
      console.error('[ProductService.listByShop] Error fetching products', { shopId, error: e });
  ...
  async listAllActive(paging?: { page?: number; limit?: number }) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('[ProductService.listAllActive] Error fetching products', { error: e });
      throw this.mapDbErrorToBadRequest(e);
    }
  }
