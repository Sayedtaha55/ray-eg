...
## 2025-05-22 - [Optimized Local State Lookups]
**Learning:** Frequent `localStorage` access and `JSON.parse` operations during React render cycles (e.g., for favorite status in lists) can become a significant bottleneck as the dataset grows. In-memory caching with a `Set` provides O(1) lookups and significantly reduces I/O overhead.
**Action:** Use a central in-memory cache (like a `Set` or `Map`) for frequently accessed `localStorage` data, and synchronize it across tabs using the `storage` event.
