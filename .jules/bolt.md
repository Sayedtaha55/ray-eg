...
## 2026-03-01 - [Image Optimization Pattern]
**Learning:** The codebase has a built-in optimization pattern using `SmartImage` for lazy loading/skeletons and `getOptimizedImageUrl` for serving resolution-appropriate variants (`md`, `thumb`). However, this pattern was inconsistently applied in core components like `ProductCard`.
**Action:** When working with product or gallery listings, always leverage `SmartImage` and `getOptimizedImageUrl('md'|'thumb')` to ensure minimal bandwidth usage and smooth loading states.
