...
## 2025-05-20 - [Favorites Cache Optimization]
**Learning:** Frequent access to `localStorage` with `JSON.parse` in render loops (like checking `isFavorite` for every item in a product list) is a significant bottleneck. O(N) array lookups further degrade performance as the list grows.
**Action:** Implement an in-memory `Set` cache for frequently accessed `localStorage` data. This provides O(1) lookups and eliminates redundant parsing. Ensure cross-tab synchronization by listening to the `storage` window event.
