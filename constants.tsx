...
const _favsCache = new Set<string>();
let _favsInitialized = false;

const initFavsCache = () => {
  if (typeof window === 'undefined') return;
  if (_favsInitialized) return;
  try {
    ...
    if (Array.isArray(parsed)) {
      parsed.forEach((id) => {
        ...
        if (sid) _favsCache.add(sid);
      });
    }
    _favsInitialized = true;
  } catch {
    _favsInitialized = true;
  }
};
...
export const RayDB = {
  ...
  getFavorites: () => {
    initFavsCache();
    return Array.from(_favsCache);
  },
  isFavorite: (id: string) => {
    initFavsCache();
    return _favsCache.has(String(id || '').trim());
  },
  ...
  toggleFavorite: (id: string) => {
    ...
    if (!sid) return false;
    initFavsCache();
    const isNowFav = !_favsCache.has(sid);
    if (isNowFav) _favsCache.add(sid); else _favsCache.delete(sid);
    localStorage.setItem('ray_favorites', JSON.stringify(Array.from(_favsCache)));
    window.dispatchEvent(new Event('ray-db-update'));
    return isNowFav;
  },
