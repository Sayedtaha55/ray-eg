...
// In-memory cache for favorites to avoid repeated localStorage access and JSON parsing
// This provides O(1) lookups and significantly boosts performance in lists
let favoritesCache: Set<string> | null = null;

const refreshFavoritesCache = () => {
  try {
    const raw = typeof localStorage !== 'undefined' ? localStorage.getItem('ray_favorites') : null;
...
// Listen for storage events to sync favorites across tabs
...
export const RayDB = {
  ...
  getFavorites: () => {
    if (!favoritesCache) refreshFavoritesCache();
    return Array.from(favoritesCache || []);
  },
  isFavorite: (id: string) => {
    if (!favoritesCache) refreshFavoritesCache();
    return favoritesCache?.has(String(id)) || false;
  },
  ...
  toggleFavorite: (id: string) => {
    if (!favoritesCache) refreshFavoritesCache();
    const sid = String(id);
    const isFav = favoritesCache?.has(sid);
