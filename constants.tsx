...
// Memory cache for favorites to avoid redundant JSON.parse and O(N) lookups
...
if (typeof window !== 'undefined') {
  try {
    ...
    if (Array.isArray(favs)) {
      favs.forEach((id) => favoritesCache.add(String(id)));
    }
  } catch {
    // ignore parse errors
  }
...
export const RayDB = {
  ...
  toggleFavorite: (id: string) => {
    ...
    const nextArr = Array.from(favoritesCache);
    localStorage.setItem('ray_favorites', JSON.stringify(nextArr));
    window.dispatchEvent(new Event('ray-db-update'));
    return !isFav;
  },
