...
const getFavoritesSet = (): Set<string> => {
...
export const RayDB = {
  ...
  getFavorites: () => Array.from(getFavoritesSet()),
  isFavorite: (id: string) => getFavoritesSet().has(String(id)),
  ...
  toggleFavorite: (id: string) => {
    ...
    const isNowFavorite = !favs.has(sid);
    if (isNowFavorite) favs.add(sid); else favs.delete(sid);
    if (typeof window !== 'undefined') {
      localStorage.setItem('ray_favorites', JSON.stringify(Array.from(favs)));
