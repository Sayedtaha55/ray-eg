...
export function persistSession(session: { user?: any; accessToken?: string }) {
  try {
    ...
    // SECURITY: Only store token in localStorage as fallback when httpOnly cookie is not available.
    // httpOnly cookies are preferred because they are not accessible via JavaScript (XSS protection).
    if (session?.accessToken && !hasSessionCookie()) {
      localStorage.setItem(TOKEN_KEY, session.accessToken);
    }
    // If httpOnly cookie is now available, remove any stale localStorage token
    if (hasSessionCookie()) {
