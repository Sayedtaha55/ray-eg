...
export async function getCustomerAnalyticsViaBackend(shopId: string) {
  return await backendGet<any>(`/api/v1/customers/shop/${shopId}/analytics`);
}

export async function getCustomerDetailViaBackend(shopId: string, customerId: string) {
  return await backendGet<any>(`/api/v1/customers/shop/${shopId}/${customerId}/detail`);
}
