...
import {
  ...
  getCustomerAnalyticsViaBackend,
  getCustomerDetailViaBackend,
...
import {
  createSupportTicketPublicViaBackend,
  createSupportTicketViaBackend,
  listMySupportTicketsViaBackend,
  listSupportTicketsAdminViaBackend,
  getSupportStatsAdminViaBackend,
  replySupportTicketAdminViaBackend,
  updateSupportTicketStatusAdminViaBackend,
  updateSupportTicketPriorityAdminViaBackend,
  deleteSupportTicketAdminViaBackend,
...
export const ApiService = {
  ...
  // Support Tickets (Customer Service & Complaints)
  submitSupportTicket: async (payload: { type?: string; subject?: string; message?: string; userName?: string; userEmail?: string; userPhone?: string; shopId?: string; orderId?: string }) => {
    return await createSupportTicketPublicViaBackend(payload);
  },
  submitMySupportTicket: async (payload: { type?: string; subject?: string; message?: string; shopId?: string; orderId?: string }) => {
    return await createSupportTicketViaBackend(payload);
  },
  getMySupportTickets: async (opts?: { take?: number; skip?: number }) => {
    return await listMySupportTicketsViaBackend(opts);
  },
  getSupportTickets: async (opts?: { take?: number; skip?: number; status?: string; type?: string; q?: string }) => {
    return await listSupportTicketsAdminViaBackend(opts);
  },
  getSupportStats: async () => {
    return await getSupportStatsAdminViaBackend();
  },
  replySupportTicket: async (id: string, reply: string) => {
    return await replySupportTicketAdminViaBackend(id, reply);
  },
  updateSupportTicketStatus: async (id: string, status: string) => {
    return await updateSupportTicketStatusAdminViaBackend(id, status);
  },
  updateSupportTicketPriority: async (id: string, priority: string) => {
    return await updateSupportTicketPriorityAdminViaBackend(id, priority);
  },
  deleteSupportTicket: async (id: string) => {
    return await deleteSupportTicketAdminViaBackend(id);
  },
  ...
  getCustomerAnalytics: async (shopId: string) => {
    const sid = String(shopId || '').trim();
    if (!sid) return null;
    try {
      return await getCustomerAnalyticsViaBackend(sid);
    } catch {
      return null;
    }
  },

  getCustomerDetail: async (shopId: string, customerId: string) => {
    const sid = String(shopId || '').trim();
    const cid = String(customerId || '').trim();
    if (!sid || !cid) return null;
    try {
      return await getCustomerDetailViaBackend(sid, cid);
    } catch {
      return null;
    }
  },
  ...
  aiGenerateTheme: async (params: {
    ...
    return await aiGenerateTheme(params);
  },
  aiGeneratePages: async (params: {
    shopId: string;
    activityId: string;
    shopName: string;
    shopDescription?: string;
    ...
    return await aiGeneratePages(params);
  },
  aiGenerateBrand: async (params: {
    shopId: string;
    activityId: string;
    shopName: string;
    shopDescription?: string;
    ...
    return await aiGenerateBrand(params);
  },
  aiBuilderChat: async (params: {
    shopId: string;
    message: string;
    context?: { currentPage?: string; locale?: string; activityId?: string; selectedSectionId?: string };
  }) => {
    return await aiBuilderChat(params);
  },

  // AI Visual Editor — Element-specific AI changes
  aiVisualEdit: async (params: {
    shopId: string;
    ...
    return await aiVisualEdit(params);
  },
  ...
  updateShopProvider: async (id: string, updates: any): Promise<any> => {
    try {
      const activityType = updates.activityType || 'general';
      const key = `ray_providers_${activityType}`;
      const raw = localStorage.getItem(key);
      if (!raw) return null;
  ...
  updateShopService: async (id: string, updates: any): Promise<any> => {
    try {
      const activityType = updates.activityType || 'general';
      const key = `ray_services_${activityType}`;
      const raw = localStorage.getItem(key);
