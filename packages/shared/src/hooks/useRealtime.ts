...
export interface UseRealtimeOptions {
  ...
  onTyping?: (data: { shopId: string; userId: string; name: string; isTyping: boolean }) => void;
  onPresenceUpdate?: (data: { shopId: string; userId: string; name: string; role: string; online: boolean }) => void;
}
...
function getSocketUrl(): string {
  // Resolve base the same way as HTTP client (prefer explicit VITE_*)
  const metaEnv = (typeof import.meta !== 'undefined' ? (import.meta as any)?.env : undefined) || {};
  const envBackend = String(metaEnv.VITE_BACKEND_URL || metaEnv.VITE_API_URL || metaEnv.VITE_API_BASE_URL || '').trim();
  const electronBackend = typeof window !== 'undefined' ? String((window as any)?.electronApp?.backendUrl || '') : '';
  let configured = (electronBackend || envBackend).replace(/\/+$/, '');

  if (configured) {
    // Strip trailing api segments so we get the origin for WS
    configured = configured.replace(/\/api\/v1$/i, '').replace(/\/api$/i, '');
    if (configured) return configured;
  }

  // Dev default must match backend PORT (4000), not 3001
  if (typeof import.meta !== 'undefined' && metaEnv.DEV) {
...
export function useRealtime(options: UseRealtimeOptions) {
  ...
  return {
    ...
    subscribe,
    unsubscribe,
    sendTyping: (shopId: string, isTyping: boolean) => {
      socketRef.current?.emit(isTyping ? 'typing:start' : 'typing:stop', { shopId });
    },
    joinPresence: (shopId: string) => {
      socketRef.current?.emit('presence:join', { shopId });
    },
    leavePresence: (shopId: string) => {
      socketRef.current?.emit('presence:leave', { shopId });
    },
