                          ...
                          {opt.variants?.map((v: any) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (isSelected) {
                                    setSelectedAddons((prev: any[]) => prev.filter((a) => !(a.optionId === opt.id && a.variantId === v.id)));
                                  } else {
                                    setSelectedAddons((prev: any[]) => [...prev.filter((a) => a.optionId !== opt.id), { optionId: opt.id, variantId: v.id }]);
                                  }
                                }}
...
import re
pattern = re.compile(r'{\/\* Addons Selection - simplified \*\/}\s+{addonsDef\.length > 0 && \(.*?\)\s+}', re.DOTALL)
content = pattern.sub(new_addons_ui, content)
