...
const ProductCard = React.memo(function ProductCard({
  ...
  const motionProps = disableMotion || isLowEnd ? {} : {
      ...
      <>
        ...
        >
          ...
          <SmartImage
            ...
            imgClassName={`w-full h-full ${effectiveImageFit === 'contain' ? 'object-contain bg-slate-50' : 'object-cover'} ${!isLowEnd ? 'group-hover:scale-110 transition-transform duration-[1s]' : ''} ${imageReady ? 'opacity-100' : 'opacity-0'}`}
            optimizeVariant="md"
        ...
        >
          ...
          {cardMediaMode === '3d' && has3D ? (
            ...
            <img
              ...
              decoding="async"
              src={product.imageUrl || (product as any).image_url}
              className={`w-full h-full ${effectiveImageFit === 'contain' ? 'object-contain bg-slate-50' : 'object-cover'} ${!isLowEnd ? 'group-hover:scale-110 transition-transform duration-[1s]' : ''} ${imageReady ? 'opacity-100' : 'opacity-0'}`}
              style={{ transitionProperty: 'opacity, transform' }}
              alt={product.name}
              onLoad={(e) => {
                setImageReady(true);
