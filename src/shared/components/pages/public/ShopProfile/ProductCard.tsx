...
import { coerceBoolean, hexToRgba, IS_LOW_END_DEVICE } from './utils';
      ...
      <>
        ...
        >
          ...
          <SmartImage
            ...
            imgClassName={`w-full h-full ${effectiveImageFit === 'contain' ? 'object-contain bg-slate-50' : 'object-cover'} ${!IS_LOW_END_DEVICE ? 'group-hover:scale-110 transition-transform duration-[1s]' : ''} ${imageReady ? 'opacity-100' : 'opacity-0'}`}
            optimizeVariant="md"
        ...
        >
          ...
          {cardMediaMode === '3d' && has3D ? (
            ...
            <img
              ...
              decoding="async"
              src={product.imageUrl || (product as any).image_url}
              className={`w-full h-full ${effectiveImageFit === 'contain' ? 'object-contain bg-slate-50' : 'object-cover'} ${!IS_LOW_END_DEVICE ? 'group-hover:scale-110 transition-transform duration-[1s]' : ''} ${imageReady ? 'opacity-100' : 'opacity-0'}`}
              style={{ transitionProperty: 'opacity, transform' }}
              alt={product.name}
              onLoad={(e) => {
                setImageReady(true);
