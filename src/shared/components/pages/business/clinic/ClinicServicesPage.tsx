...
const ClinicServicesPage: React.FC<Props> = ({ shop, onSaved }) => {
  ...
  const saveSpecialtiesList = async (nextList: Specialty[]) => {
    ...
    try {
      ...
      setSuccessMsg(`تم تحديث قائمة ${vocab.servicePlural} بنجاح!`);
      if (onSaved) onSaved();
  ...
  const handleAddOrEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setErrorMsg(`الرجاء إدخال اسم ${vocab.serviceSingular}`);
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(`هل أنت متأكد من رغبتك في حذف ${vocab.serviceSingular}؟`)) return;
  ...
  return (
    <div className={context.shop !== undefined ? "text-right" : "bg-white rounded-[3.5rem] border border-slate-100 shadow-sm p-8 md:p-12 text-right"} dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 border-b border-slate-100 pb-6 flex-row-reverse">
        {context.shop === undefined ? (
          <div className="text-right">
            <div className="flex items-center gap-2.5 justify-end">
              ...
              <h3 className="text-2xl md:text-3xl font-black text-slate-900">{`إدارة ${vocab.servicePlural}`}</h3>
            </div>
            <p className="text-slate-400 font-bold text-xs md:text-sm mt-2">
              {`حدد ${vocab.servicePlural} المتاحة، والظاهرة في حقل حجز مواعيد ${vocab.customerPlural} بالخارج.`}
            </p>
          </div>
        ) : <div />}
        <button
        ...
        >
          <Plus size={18} />
          <span>{`إضافة ${vocab.serviceSingular} جديدة`}</span>
        </button>
      </div>
            ...
            >
              ...
              <div className="absolute top-4 left-4 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  title={`تعديل ${vocab.serviceSingular}`}
                >
                  <Edit2 size={13} />
                </button>
                <button
                  ...
                  title={`حذف ${vocab.serviceSingular}`}
                >
                  <Trash2 size={13} />
                </button>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-bold text-slate-400">{vocab.serviceSingular}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-250">
            ...
            <div className="relative p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 flex-row-reverse text-right">
              <div>
                <h3 className="text-xl font-black text-slate-900">
                  {editingId ? `تعديل ${vocab.serviceSingular}` : `إضافة ${vocab.serviceSingular} جديدة`}
                </h3>
                <p className="text-xs font-bold text-slate-400 mt-1">{`تعديل ${vocab.servicePlural} المعروضة بجدول الحجوزات`}</p>
              </div>
              <button
            ...
            <form onSubmit={handleAddOrEditSubmit} className="p-6 sm:p-8 space-y-4 text-right" dir="rtl">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 mb-1.5">{`اسم ${vocab.serviceSingular}`}</label>
                  <input
                    type="text"
                    required
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder={`مثال: اسم ${vocab.serviceSingular}`}
