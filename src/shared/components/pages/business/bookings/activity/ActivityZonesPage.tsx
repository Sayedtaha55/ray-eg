    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">مناطق الخدمة</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{zones.filter(z => z.isActive).length} منطقة نشطة</p>
          </div>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">إضافة منطقة خدمة</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">اسم المنطقة *</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">رسوم الانتقال (ج.م)</label>
              <input type="number" value={form.deliveryFee} onChange={e => setForm(f => ({ ...f, deliveryFee: e.target.value }))}
                placeholder="50"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">الأحياء (مفصولة بفاصلة)</label>
              <input type="text" value={form.areas} onChange={e => setForm(f => ({ ...f, areas: e.target.value }))}
                placeholder="العليا, السليمانية, الملز"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">الوقت المتوقع</label>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="ابحث في المناطق أو الأحياء..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لم تضف مناطق خدمة بعد</p>
        </div>
      ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-3 mb-2 text-xs font-bold flex-wrap">
                      <span className={`px-2.5 py-1 rounded-lg ${zone.deliveryFee === 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                        {zone.deliveryFee === 0 ? 'مجاني' : `${zone.deliveryFee} ج.م رسوم`}
                      </span>
