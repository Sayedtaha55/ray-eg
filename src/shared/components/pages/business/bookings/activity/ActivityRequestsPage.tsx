    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">طلبات العملاء</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{requests.filter(r => r.status === 'pending').length} طلب قيد المراجعة</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-row-reverse">
        {[
          ...
          { key: 'pending' as const, label: 'قيد المراجعة', count: requests.filter(r => r.status === 'pending').length },
          { key: 'approved' as const, label: 'موافق', count: requests.filter(r => r.status === 'approved').length },
          { key: 'rejected' as const, label: 'مرفوض', count: requests.filter(r => r.status === 'rejected').length },
        ].map(tab => (
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="ابحث في الطلبات..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لا توجد طلبات</p>
        </div>
      ) : (
