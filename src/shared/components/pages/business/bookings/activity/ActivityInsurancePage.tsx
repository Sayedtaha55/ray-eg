    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">التأمين والشروط</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{plans.length} خطة تأمين • {plans.filter(p => p.isActive).length} نشطة</p>
          </div>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">إضافة خطة تأمين</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">اسم الخطة *</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="مثال: تأمين شامل"
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">السعر اليومي (ج.م)</label>
              <input type="number" value={form.pricePerDay} onChange={e => setForm(f => ({ ...f, pricePerDay: e.target.value }))}
                placeholder="75"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">مبلغ الخصم (ج.م)</label>
              <input type="number" value={form.deductible} onChange={e => setForm(f => ({ ...f, deductible: e.target.value }))}
                placeholder="500"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">التغطيات (مفصولة بفاصلة)</label>
              <input type="text" value={form.includes} onChange={e => setForm(f => ({ ...f, includes: e.target.value }))}
                placeholder="حوادث, سرقة, حريق"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-black text-slate-600 mb-1.5">وصف التغطية</label>
            <textarea value={form.coverage} onChange={e => setForm(f => ({ ...f, coverage: e.target.value }))}
              rows={2} placeholder="تفاصيل التغطية..."
      ...
      {plans.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لم تضف خطط تأمين بعد</p>
        </div>
      ) : (
