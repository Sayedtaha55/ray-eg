...
const LABELS: Record<string, { title: string; type: string }> = {
    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold mt-0.5">{subs.filter(s => s.isActive).length} {lbl.type} نشط</p>
          </div>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          ...
          <Plus size={16} /> إضافة {lbl.type}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">إضافة {lbl.type} جديد</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">المدة</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">حصص/أسبوع</label>
              <input type="number" value={form.sessionsPerWeek} onChange={e => setForm(f => ({ ...f, sessionsPerWeek: e.target.value }))}
                placeholder="3" min="1"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">الحد الأقصى للأعضاء</label>
              <input type="number" value={form.maxMembers} onChange={e => setForm(f => ({ ...f, maxMembers: e.target.value }))}
                placeholder="50"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">المميزات (فاصلة)</label>
              <input type="text" value={form.features} onChange={e => setForm(f => ({ ...f, features: e.target.value }))}
                placeholder="مدرب خاص, خزانة, ساونا"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لم تضف اشتراكات بعد</p>
        </div>
      ) : (
            ...
            return (
              <div key={sub.id} className={`bg-white rounded-[2rem] border shadow-sm p-5 space-y-3 hover:shadow-md transition-all ${sub.isActive ? 'border-slate-100' : 'border-red-100 opacity-60'}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="flex items-center gap-1 text-sm font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg"><DollarSign size={13} /> {sub.price} ج.م</span>
                  ...
                  {sub.sessionsPerWeek && <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg">{sub.sessionsPerWeek} حصص/أسبوع</span>}
                </div>
                {sub.maxMembers && (
                  <div>
                ...
                <div className="flex gap-2 pt-1">
                  <button className="flex-1 py-2 rounded-xl border border-slate-200 text-xs font-black text-slate-600 hover:bg-slate-50 flex items-center justify-center gap-1.5"><Edit2 size={12} /> تعديل</button>
