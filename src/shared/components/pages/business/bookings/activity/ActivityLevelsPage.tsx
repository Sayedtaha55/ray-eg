    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">المستويات التعليمية</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{levels.length} مستوى • {totalStudents} طالب</p>
          </div>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">إضافة مستوى جديد</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">اسم المستوى *</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">اللون</label>
            ...
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">المتطلبات المسبقة</label>
              <input type="text" value={form.prerequisites} onChange={e => setForm(f => ({ ...f, prerequisites: e.target.value }))}
                placeholder="مثال: إتمام المبتدئ"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-right focus:outline-none focus:border-cyan-300" />
            </div>
          </div>
      ...
      {levels.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لم تضف مستويات بعد</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 flex-wrap text-xs font-bold">
                    <span className="text-slate-500 bg-slate-50 px-2.5 py-1 rounded-lg flex items-center gap-1"><Users size={11} /> {level.studentCount} طالب</span>
