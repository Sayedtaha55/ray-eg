    ...
    <div className="space-y-5 text-right" dir="rtl">
      <div className="flex items-center justify-between flex-row-reverse">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-slate-900">التذاكر والسعات</h2>
            <p className="text-xs text-slate-400 font-bold mt-0.5">{totalSold} تذكرة مباعة • {totalRevenue.toLocaleString()} ج.م إيرادات</p>
          </div>
        </div>
        <button onClick={() => setShowForm(!showForm)}
          ...
          <Plus size={16} /> إضافة تذكرة
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-[2rem] border border-cyan-100 shadow-sm p-6 space-y-4">
          <h3 className="font-black text-slate-900 text-sm">إضافة نوع تذكرة</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-black text-slate-600 mb-1.5">نوع التذكرة *</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                placeholder="مثال: تذكرة VIP"
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="ابحث في التذاكر..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 text-center">
          ...
          <p className="font-bold text-slate-400">لم تضف تذاكر بعد</p>
        </div>
      ) : (
