...
const ImageUploadSection: React.FC<ImageUploadSectionProps> = ({
  ...
  return (
    <div className="space-y-3">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block pr-4">{t('business.products.productImage')}</label>
      <div
      ...
      >
        {imagePreview ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <div className="bg-white/90 px-6 py-3 rounded-2xl font-black text-xs flex items-center gap-2">
                <Upload size={16} /> {t('business.products.changeImage')}
              </div>
            </div>
          </>
        ) : (
          <div className="text-center p-8">
            ...
            <p className="font-black text-slate-900 mb-1">{t('business.products.clickToUpload')}</p>
