    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-sm">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 flex-row-reverse">
        <div>
          ...
          <p className="text-slate-400 font-black text-sm mt-2">
            عند تحويل الحجز لـ "تم الاستلام" سيتم إضافة العميل تلقائياً لقاعدة العملاء
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="bg-amber-100 text-amber-600 px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest">
            {pendingCount} طلب ينتظر
          </span>
          <span className="bg-green-100 text-green-600 px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest">
            {completedCount} منفذ
          </span>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {filteredReservations.length === 0 ? (
          ...
          filteredReservations.map((res) => (
            <div
              key={res.id}
              className="bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col lg:flex-row items-center justify-between gap-8 hover:bg-slate-50 transition-all group"
            >
              <div className="flex items-center gap-8 flex-row-reverse w-full lg:w-auto">
                <img src={(res as any).itemImage} className="w-24 h-24 rounded-3xl object-cover shadow-xl group-hover:rotate-3 transition-transform" />
                <div className="text-right">
                  <p className="font-black text-2xl text-slate-900 mb-2">{(res as any).itemName}</p>
                  <div className="space-y-1">
                    <p className="text-slate-500 font-black text-sm flex items-center justify-end gap-2">
                      <User size={14} /> العميل: {(res as any).customerName}
                    </p>
                    <p className="text-slate-400 font-bold text-sm flex items-center justify-end gap-2">
                ...
                {normalizeReservationStatus((res as any).status) === 'pending' ? (
                  <div className="flex gap-3 w-full md:w-auto">
                    <button
                      ...
                      className="flex-1 md:w-40 py-5 bg-green-500 text-white rounded-2xl font-black text-xs hover:bg-green-600 transition-all shadow-lg shadow-green-100 flex items-center justify-center gap-2"
                    >
                      <UserCheck size={14} /> تم الاستلام
                    </button>
                    <button
                      onClick={() => onUpdateStatus(res.id, 'expired')}
                      className="flex-1 md:w-40 py-5 bg-white border border-slate-200 text-red-500 rounded-2xl font-black text-xs hover:bg-red-50 transition-all"
                    >
                      إلغاء الحجز
                    </button>
                  </div>
