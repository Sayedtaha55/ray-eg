...
import { X, Send, Bot, User, Loader2, Sparkles, Crown, Zap, ChevronUp } from 'lucide-react';
...
const AiAssistantPanel: React.FC<AiAssistantPanelProps> = ({
  ...
  const handleSend = async (messageText?: string) => {
    ...
    try {
      ...
      // If actions were executed, notify parent
      if (response.actions?.some((a) => a.confirmed) && onActionExecuted) {
        onActionExecuted();
      }
    } catch (err: any) {
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-l from-cyan-500 to-cyan-600 text-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-black text-sm leading-tight">{t('ai.assistantTitle')}</h3>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-slate-50/50">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-2">
                  ...
                  <div>
                    <p className="font-black text-slate-700 text-sm">{t('ai.welcomeTitle')}</p>
                    <p className="text-xs text-slate-400 mt-1">{t('ai.welcomeSubtitle')}</p>
                  </div>
              ...
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex gap-2 ${msg.role === 'user' ? (isArabic ? 'flex-row-reverse' : 'flex-row-reverse') : ''}`}
                >
                  <div
                ...
                <div className="flex gap-2">
                  ...
                  <div className="bg-white border border-slate-100 rounded-2xl px-4 py-3 shadow-sm">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-slate-400 font-bold">{t('ai.thinking')}</span>
                    </div>
                  </div>
                </div>
              )}
