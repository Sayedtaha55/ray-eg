...
const ShoppingModeSection: React.FC<Props> = ({ config, setConfig }) => {
  ...
  const shoppingItems = [
    { key: 'productCardAddToCart', label: t('business.builder.shoppingMode.addToCart') },
    ...
    <div className="space-y-6">
      <div className="bg-slate-50 rounded-2xl p-4 space-y-2">
        <div className="flex items-center gap-2 text-slate-600">
          ...
          <span className="text-xs font-black">{t('business.builder.shoppingMode.description')}</span>
        </div>
      </div>
        ...
        {shoppingItems.map((item) => (
          <div key={item.key} className="flex items-center justify-between">
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-all ${isVisible(item.key) ? 'bg-[#00E5FF]/10 text-[#00E5FF]' : 'bg-slate-100 text-slate-400'}`}
            >
              {isVisible(item.key) ? <Eye size={16} /> : <EyeOff size={16} />}
            </button>
          </div>
        ))}
      </div>
