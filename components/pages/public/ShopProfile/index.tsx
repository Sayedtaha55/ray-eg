...
const ShopProfile: React.FC = () => {
  ...
  const description = shopDescription
    ? shopDescription.substring(0, 160)
    : `استكشف ${shop?.name}، أفضل ${shop?.category === 'RESTAURANT' ? 'مطعم' : 'متجر'} في ${shop?.city} يقدم مجموعة واسعة من المنتجات والخدمات.`;
  ...
  if (error || !shop)
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center" dir="rtl">
        ...
        <button
          onClick={() => navigate('/')}
          className="px-8 py-4 md:px-10 md:py-5 bg-slate-900 text-white rounded-full font-black flex items-center gap-3 shadow-xl"
        >
  ...
  const productDisplayMode = (currentDesign!.productDisplay ||
    ((currentDesign as any).productDisplayStyle === 'list' ? 'list' : undefined) ||
    'cards') as any;
  ...
  return (
    <>
      ...
      >
          ...
          <div className="max-w-[1400px] mx-auto px-4 md:px-12 py-3 md:py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div>
                  <h3
                    className={`font-black ${isBold ? 'text-lg md:text-2xl' : 'text-sm md:text-lg'}`}
                ...
                {showHeaderChatButton && whatsappHref && (
                  <a
                    ...
                    style={{
                      ...
                      color: '#000',
                      borderColor: `${currentDesign!.primaryColor}20`,
                    }}
                  >
            ...
            {(showHeaderNav || showHeaderShareButton) && isHeaderMenuOpen && (
              <>
                <div className="fixed inset-0 z-[110] md:hidden" onClick={() => setIsHeaderMenuOpen(false)} />
                  ...
                  <div className="rounded-2xl border border-slate-100 bg-white/95 backdrop-blur-md shadow-lg overflow-hidden">
                    {showHeaderNavHome && (
                      <button
                        ...
                        className={`w-full text-right px-4 py-3 font-black text-sm transition-colors ${
                          activeTab === 'products' ? 'bg-slate-50' : 'bg-transparent'
                        }`}
                    ...
                    {showHeaderNavGallery && (
                      <button
                        ...
                        className={`w-full text-right px-4 py-3 font-black text-sm transition-colors ${
                          activeTab === 'gallery' ? 'bg-slate-50' : 'bg-transparent'
                        }`}
                        style={{ color: headerTextColor }}
                      >
                        معرض الصور
                    ...
                    {showHeaderNavInfo && (
                      <button
                        ...
                        className={`w-full text-right px-4 py-3 font-black text-sm transition-colors ${
                          activeTab === 'info' ? 'bg-slate-50' : 'bg-transparent'
                        }`}
                        style={{ color: headerTextColor }}
                      >
                        معلومات المتجر
        ...
        <section
          className={`relative transition-all duration-1000 overflow-hidden bg-slate-900 ${
            isBold
              ? 'h-[45vh] md:h-[85vh] m-0 md:m-6 md:rounded-[4.5rem] shadow-2xl'
              : isMinimal
                ? 'h-[25vh] md:h-[45vh]'
                : 'h-[35vh] md:h-[60vh]'
          }`}
          ...
          {false && (
            <div className="absolute bottom-6 md:bottom-10 left-0 right-0 flex justify-center px-6">
              <button
                ...
                className="bg-white/95 backdrop-blur-sm text-slate-900 px-6 py-3 md:px-8 md:py-4 rounded-full font-black text-xs md:text-base flex items-center gap-2 md:gap-3 shadow-2xl active:scale-95 transition-all border border-slate-100 hover:gap-5"
              >
        ...
        <div
          className={`max-w-[1400px] mx-auto px-4 md:px-12 relative z-10 pb-16 md:pb-24 ${
            isBold ? '-mt-16 md:-mt-48' : '-mt-10 md:-mt-24'
          }`}
        >
          <div
            className={`flex flex-col items-center md:items-end md:flex-row-reverse gap-4 md:gap-16 ${
              isMinimal ? 'md:items-center' : ''
            }`}
          >
            <MotionDiv
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              className={`bg-white p-1 md:p-1.5 shadow-2xl shrink-0 ring-4 ring-white ${
                isBold
                  ? 'rounded-[2rem] md:rounded-[3rem] w-24 h-24 md:w-64 md:h-64 rotate-3'
                  : isMinimal
                    ? 'rounded-xl md:rounded-2xl w-20 h-20 md:w-48 md:h-48'
                    : 'rounded-full w-24 h-24 md:w-56 md:h-56'
              }`}
            >
              {shopLogoSrc ? (
                <img
                  ...
                  className={`w-full h-full object-cover ${
                    isBold ? 'rounded-[1.8rem] md:rounded-[2.5rem]' : isMinimal ? 'rounded-lg md:rounded-xl' : 'rounded-full'
                  }`}
                  alt={shop.name}
                />
              ) : (
            ...
            <div className={`flex-1 text-center md:text-right ${isMinimal ? 'md:text-center' : ''}`}>
              <div
                className={`flex items-center justify-center md:justify-start gap-2 mb-2 md:mb-3 flex-row-reverse ${
                  isMinimal ? 'md:justify-center' : ''
                }`}
              >
                {isRestaurant ? (
                  <Utensils size={14} className="md:w-4 md:h-4 text-[#BD00FF]" />
                ) : (
                  <ShoppingBag size={14} className="md:w-4 md:h-4 text-[#00E5FF]" />
                )}
                <span className="text-slate-400 font-black text-[9px] md:text-xs uppercase tracking-widest">
                  {shop.category} • {shop.city}
                </span>
              </div>
              <h1
                className={`font-black tracking-tighter mb-4 md:mb-5 leading-tight ${
                  isBold
                    ? 'text-3xl md:text-8xl lg:text-[10rem]'
                    : isMinimal
              ...
              <div
                className={`flex flex-wrap items-center justify-center md:justify-start gap-2 md:gap-4 mb-8 ${
                  isMinimal ? 'md:justify-center' : ''
                }`}
              >
                ...
                {showShopFollowButton && (
                  <button
                    ...
                    className={`px-6 py-1.5 md:px-8 md:py-2.5 rounded-full font-black text-[11px] md:text-sm transition-all shadow-xl active:scale-95 disabled:opacity-60 ${
                      hasFollowed ? 'bg-green-500 text-white' : 'bg-slate-900 text-white hover:bg-black'
                    }`}
                  >
            ...
            {activeTab === 'products' ? (
                ...
                <div className="flex overflow-x-auto no-scrollbar gap-2 mb-8 md:mb-10 pb-2">
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`px-5 py-2 md:px-6 md:py-2.5 rounded-full font-black text-[10px] md:text-xs whitespace-nowrap transition-all border ${
                        activeCategory === cat
                  ...
                  className={`${
                    productDisplayMode === 'list'
                      ...
                      : productDisplayMode === 'minimal'
                        ...
                        : `grid gap-3 md:gap-8 ${isMinimal ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`
                  }`}
                >
              ...
              <MotionDiv key="gallery-view" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <div className="mb-8 md:mb-10">
                  <h2
                    className={`font-black mb-4 md:mb-6 ${isBold ? 'text-2xl md:text-4xl' : 'text-xl md:text-3xl'}`}
                    style={{ color: currentDesign!.primaryColor }}
                  >
                    معرض {shop.name}
                  </h2>
                  <p className="text-slate-600 text-sm md:text-base">استكشف صور ومعارض من {shop.name}</p>
                </div>
                {galleryTabLoading ? (
              ...
              >
                <div
                  className={`p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 space-y-6 md:space-y-8 ${
                    isMinimal ? 'bg-white' : 'bg-slate-50'
                  }`}
                >
                ...
                <div
                  className={`p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 flex flex-col justify-center items-center text-center space-y-6 ${
                    isMinimal ? 'bg-white' : 'bg-slate-50'
                  }`}
                >
                  <div
                    className={`w-16 h-16 md:w-24 md:h-24 bg-white rounded-2xl md:rounded-[2rem] flex items-center justify-center shadow-xl mb-2 ${
                      isBold ? 'rotate-6' : ''
                    }`}
                  >
                  ...
                  <p className="text-slate-500 font-bold max-w-xs leading-relaxed text-xs md:text-base">
        ...
        <AnimatePresence>
          {spatialMode && (
            <MotionDiv
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[200] bg-black/98 backdrop-blur-3xl flex flex-col items-center justify-center p-6 md:p-8 text-center text-white"
            >
              ...
              <h2 className="text-3xl md:text-7xl font-black mb-4 md:mb-6 tracking-tighter">
                جاري تهيئة الشعاع المكاني...
              </h2>
              ...
              <button
                ...
                className="px-10 py-4 md:px-16 md:py-6 bg-white text-black rounded-full font-black text-lg md:text-2xl active:scale-95 transition-all shadow-[0_0_50px_rgba(255,255,255,0.2)]"
              >
                العودة للواقع
              </button>
            </MotionDiv>
        ...
        {showFooter && (
          ...
          >
            <div className="max-w-[1400px] mx-auto px-4 md:px-12 py-8 md:py-12">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-8">
                <div className="text-center md:text-right">
                  <div className="flex items-center justify-center md:justify-start gap-3 mb-3">
                    <img
                      src={((shop.logoUrl || (shop as any).logo_url || '').trim() || undefined) as any}
                ...
                {showFooterQuickLinks && (
                  <div className="text-center md:text-right">
                    <h5 className="font-black mb-3 text-sm md:text-base" style={{ color: footerTextColor }}>
                      روابط سريعة
                    </h5>
              ...
              >
                <p>
                  جميع الحقوق محفوظة © {new Date().getFullYear()} {shop.name} • تطوير بواسطة منصة تست
                </p>
              </div>
            </div>
          </footer>
