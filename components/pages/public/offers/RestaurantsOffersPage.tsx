...
const RestaurantsOffersPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const offersData = await ApiService.getOffers({ take: PAGE_SIZE, skip: 0, shopCategory: 'RESTAURANT' });
    ...
    const loadMoreOffers = async () => {
      ...
      setLoadingMore(true);
      try {
        const next = await ApiService.getOffers({ take: PAGE_SIZE, skip: offersLenRef.current, shopCategory: 'RESTAURANT' });
...
export default RestaurantsOffersPage;
