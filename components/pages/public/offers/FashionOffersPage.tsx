...
const FashionOffersPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const offersData = await ApiService.getOffers({ take: PAGE_SIZE, skip: 0, shopCategory: 'FASHION' });
    ...
    const loadMoreOffers = async () => {
      ...
      setLoadingMore(true);
      try {
        const next = await ApiService.getOffers({ take: PAGE_SIZE, skip: offersLenRef.current, shopCategory: 'FASHION' });
    ...
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-4 md:py-12 relative">
      <div className="flex flex-col items-center text-center mb-8 md:mb-20">
         <MotionDiv 
            ...
            className="inline-flex items-center gap-2 px-4 md:px-6 py-2 md:py-2.5 bg-purple-500 text-white rounded-full font-black text-[9px] md:text-[10px] md:text-xs uppercase tracking-[0.2em] mb-6 md:mb-10 shadow-2xl"
         >
            <ShoppingBag className="w-3 h-3 text-white fill-current" />
            عروض الأزياء
         </MotionDiv>
         <h1 className="text-2xl md:text-4xl lg:text-8xl font-black tracking-tighter mb-4 md:mb-8 leading-[0.85]">عروض<br/><span className="text-purple-500">الملابس والأحذية.</span></h1>
         ...
         <Link
           to="/map"
           className="inline-flex items-center gap-2 px-6 py-3 md:px-8 md:py-4 bg-purple-500 text-white rounded-xl md:rounded-2xl font-black text-sm md:text-base hover:bg-purple-600 transition-all shadow-xl"
         >
        ...
        {hasMoreOffers && (
          <div className="mt-10 md:mt-16 flex items-center justify-center">
            <button
              ...
              className="px-8 py-3 md:px-10 md:py-4 bg-purple-500 text-white rounded-xl md:rounded-2xl font-black text-sm md:text-base flex items-center justify-center gap-3 hover:bg-purple-600 transition-all shadow-xl"
              disabled={loadingMore}
            >
              {loadingMore ? <Loader2 className="animate-spin" size={18} /> : null}
              <span>{loadingMore ? 'تحميل...' : 'تحميل المزيد'}</span>
...
export default FashionOffersPage;
